...
spec_path = "strategies/2026-09-14_gmma_ribbon_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("gmma_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"zscore_window": 100, "deadband": 0.25, "leverage_cap": 1.0}),
    "SPY": ("equity", {"zscore_window": 150, "deadband": 0.15, "leverage_cap": 0.4}),
    "BTC/USDT": ("crypto", {"zscore_window": 150, "deadband": 0.15, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"zscore_window": 60, "deadband": 0.25, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for zw in [60, 100, 150]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, zscore_window=zw, deadband=db,
                                        leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"zw{zw}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_gmma_ribbon_sizing.json", "w") as f:
