...
spec_path = "strategies/2026-09-14_lrs_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("lrs_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"lrs_window": 10, "deadband": 0.25, "leverage_cap": 0.4}),
    "SPY": ("equity", {"lrs_window": 10, "deadband": 0.15, "leverage_cap": 0.4}),
    "BTC/USDT": ("crypto", {"lrs_window": 20, "deadband": 0.15, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"lrs_window": 10, "deadband": 0.25, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for lw in [10, 20, 30]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, lrs_window=lw, deadband=db,
                                        leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"lw{lw}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_lrs_sizing.json", "w") as f:
