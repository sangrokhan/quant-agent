...
spec_path = "strategies/2026-09-14_cti_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("cti_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"cti_period": 14, "deadband": 0.25, "leverage_cap": 1.0, "sensitivity": 0.4}),
    "SPY": ("equity", {"cti_period": 14, "deadband": 0.35, "leverage_cap": 1.0, "sensitivity": 0.3}),
    "BTC/USDT": ("crypto", {"cti_period": 20, "deadband": 0.25, "leverage_cap": 0.4, "sensitivity": 0.6}),
    "ETH/USDT": ("crypto", {"cti_period": 20, "deadband": 0.15, "leverage_cap": 0.4, "sensitivity": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for cti_period in [14, 20, 30]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, cti_period=cti_period, deadband=db,
                                        leverage_cap=params["leverage_cap"], sensitivity=params["sensitivity"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"cti{cti_period}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_cti_sizing.json", "w") as f:
