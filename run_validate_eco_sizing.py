...
spec_path = "strategies/2026-09-16_eco_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("eco_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"eco_r": 8, "sensitivity": 1.0, "deadband": 0.20, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"eco_r": 5, "sensitivity": 0.7, "deadband": 0.25, "base_exposure": 0.5, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"eco_r": 5, "sensitivity": 0.8, "deadband": 0.20, "base_exposure": 0.25, "leverage_cap": 0.3}),
    "ETH/USDT": ("crypto", {"eco_r": 8, "sensitivity": 0.8, "deadband": 0.15, "base_exposure": 0.25, "leverage_cap": 0.25}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for er in [3, 5, 8]:
        for sens in [0.6, 0.8, 1.0]:
            r = strat.generate_returns(price_df, eco_r=er, sensitivity=sens, deadband=params["deadband"],
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"er{er}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_eco_sizing.json", "w") as f:
