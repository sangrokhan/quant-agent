...
spec_path = "strategies/2026-09-16_dpo_dist_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("dpo_strat2", spec_path)
...
configs = {
    "QQQ": ("equity", {"dpo_window": 20, "sensitivity": 0.5, "deadband": 0.3, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"dpo_window": 20, "sensitivity": 0.5, "deadband": 0.3, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"dpo_window": 10, "sensitivity": 0.6, "deadband": 0.2, "base_exposure": 0.24, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"dpo_window": 10, "sensitivity": 0.6, "deadband": 0.2, "base_exposure": 0.24, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    wf_ev = {"metric": "walk_forward_pass_fraction", "value": wf_pass_fraction, "threshold": 0.75,
             ...
             "note": "manual 4-equal-slice fallback (vbt.utils.splitting API unavailable in installed vectorbt version)"}

    param_grid_results = {}
    lev = params.get("leverage_cap", 1.0)
    base_exp = params.get("base_exposure", 0.4)
    sens_base = params["sensitivity"]
    for sens in sorted({round(sens_base - 0.2, 2), sens_base, round(sens_base + 0.2, 2)}):
        for db in [max(0.05, params["deadband"] - 0.05), params["deadband"], params["deadband"] + 0.05]:
            r = strat.generate_returns(price_df, dpo_window=params["dpo_window"], sensitivity=max(0.05, sens), deadband=db, leverage_cap=lev, base_exposure=base_exp)
            import vectorbt as vbt
...
with open("validators_dpo_dist_sizing.json", "w") as f:
