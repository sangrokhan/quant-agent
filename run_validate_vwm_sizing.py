...
spec_path = "strategies/2026-09-15_vwm_sizing_sma_trend.py"
...
configs = {
    "QQQ": ("equity", {"mom_period": 10, "smooth_period": 14, "sensitivity": 0.7, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"mom_period": 10, "smooth_period": 14, "sensitivity": 0.7, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"mom_period": 14, "smooth_period": 14, "sensitivity": 0.4, "deadband": 0.20, "base_exposure": 0.2, "leverage_cap": 0.3}),
    "ETH/USDT": ("crypto", {"mom_period": 10, "smooth_period": 14, "sensitivity": 0.4, "deadband": 0.20, "base_exposure": 0.2, "leverage_cap": 0.25}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for mp in [10, 14]:
        for sens in [0.5, 0.7]:
            r = strat.generate_returns(price_df, mom_period=mp, smooth_period=params["smooth_period"], sensitivity=sens, deadband=params["deadband"],
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"mp{mp}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_vwm_sizing.json", "w") as f:
