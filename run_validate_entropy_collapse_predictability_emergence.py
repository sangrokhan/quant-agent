...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_entropy_collapse_predictability_emergence.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": ("equity", dict(entropy_threshold=0.28, slope_window=10, max_hold_days=15)),
    "SPY": ("equity", dict(entropy_threshold=0.28, slope_window=10, max_hold_days=15)),
    "BTC/USDT": ("crypto", dict(entropy_threshold=0.28, slope_window=10, max_hold_days=15)),
}
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    pgr = {}
    for th in [0.25, 0.28, 0.3]:
        for sw in [10, 20]:
            params = dict(entropy_threshold=th, slope_window=sw, max_hold_days=15)
    ...
    with open(f"validate_result_entropy_collapse_predictability_emergence_{safe}.json", "w") as f:
