...
spec_path = os.path.join(os.path.dirname(__file__), "strategies", "2026-09-13_mar_ratio_sizing_sma_trend.py")
spec = importlib.util.spec_from_file_location("mar_strat", spec_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

start, end = datetime(2019, 1, 1), datetime(2026, 9, 1)
best_params = {"mar_reference": 0.5, "mar_window": 60}
...
for symbol, loader in [("SPY", load_equity), ("QQQ", load_equity), ("BTC/USDT", load_crypto_daily)]:
    ...
    sens_grid = {}
    for mr in [0.5, 1.0, 1.5]:
        for mw in [60, 90]:
            r = strat.generate_returns(price_df, mar_reference=mr, mar_window=mw)
            try:
                import vectorbt as vbt
                sh = r.vbt.returns(freq="D").sharpe_ratio()
            except Exception:
                sh = None
            sens_grid[str((mr, mw))] = float(sh) if sh is not None else 0.0
...
with open("validators_mar.json", "w") as f:
