...
for symbol in ["QQQ", "SPY"]:
    ...
    params = {"entry_threshold": 5.0, "exit_threshold": 70.0, "max_hold_days": 10}
    ...
    for et in [5.0, 10.0]:
        for xt in [60.0, 70.0]:
            for mh in [5, 10]:
                rr = strat.generate_returns(price_df, entry_threshold=et, exit_threshold=xt, max_hold_days=mh)
                sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
                sweep[f"et={et},xt={xt},mh={mh}"] = float(sh) if sh is not None else 0.0
...
with open("scratch_results/validate_result_rsi2_atrpct.json", "w") as f:
