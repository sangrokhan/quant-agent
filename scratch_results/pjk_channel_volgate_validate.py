...
for symbol in ["QQQ", "SPY"]:
    ...
    params = {"period": 50, "zone": 0.3, "vol_regime_ratio": 1.3}
    ...
    wf_ev = {
        ...
        "note": "manual split -- validators.check_walk_forward errors on installed vectorbt",
    }

    sweep = {}
    for vr in [1.1, 1.3, 1.6]:
        rr = strat.generate_returns(price_df, period=50, zone=0.3, vol_regime_ratio=vr)
        sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
        sweep[f"vol_regime_ratio={vr}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_pjk_channel_volgate.json", "w") as f:
