...
params = dict(entry_rsi_level=10, min_declines=2, vol_percentile_threshold=50, max_hold_days=10)
...
for symbol in ["SPY", "QQQ"]:
    ...
    sens_results = {}
    for erl in [10, 15, 20]:
        p2 = dict(params); p2["entry_rsi_level"] = erl
        r2 = strat.generate_returns(price, **p2)
        import vectorbt as vbt
        sh = r2.vbt.returns(freq="D").sharpe_ratio()
        sens_results[f"entry_rsi_{erl}"] = float(sh) if sh is not None else 0.0
    ...
    with open(f"validate_result_r3_trpercentile_{symbol.lower()}.json", "w") as f:
