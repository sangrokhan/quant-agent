...
spec = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-18_pjk_channel_trade_in_bands.py"
)
...
for symbol in ["QQQ", "SPY"]:
    ...
    # validators.py's check_walk_forward hits a vectorbt API break
    # (vbt.utils.splitting.RangeSplitter no longer exists in installed
    # vectorbt version -- also seen failing in prior iterations, e.g.
    # scratch_results/validate_result_cybernetic_osc.json). Do a manual
    # 4-way contiguous walk-forward split instead, same pass criterion
    # (fraction of splits with positive Sharpe >= 0.75).
    n_splits = 4
    idx = price_df.index
    ...
    # parameter sensitivity: small sweep around the winning (period=50, zone=0.3)
    sweep = {}
    for p in [40, 50, 60]:
        for z in [0.2, 0.3, 0.4]:
            rr = strat.generate_returns(price_df, period=p, zone=z)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"period={p},zone={z}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_pjk_channel_trade_in_bands.json", "w") as f:
