...
spec = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-20_casey_bands_meanrev.py"
)
...
params = {"atr_mult": 1.75, "entry_level": 30.0}
...
for asset_class, symbol, loader in [
    ...
    sweep = {}
    for el in [10.0, 20.0, 30.0]:
        for am in [1.0, 1.25, 1.75]:
            rr = strat.generate_returns(price_df, entry_level=el, atr_mult=am)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"el={el},am={am}"] = float(sh) if sh is not None else 0.0
