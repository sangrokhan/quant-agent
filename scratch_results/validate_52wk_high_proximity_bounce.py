...
spec = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-20_52wk_high_proximity_consolidation_bounce.py"
)
...
params = {"near_high_pct": 0.07, "adx_floor": 15.0, "stop_atr_mult": 3.0}
...
for asset_class, symbol, loader in [
    ...
    sweep = {}
    for nh in [0.05, 0.07, 0.10]:
        for af in [15.0, 20.0]:
            rr = strat.generate_returns(price_df, near_high_pct=nh, adx_floor=af, stop_atr_mult=3.0)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"nh={nh},adx={af}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_52wk_high_proximity_bounce.json", "w") as f:
