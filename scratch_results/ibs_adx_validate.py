...
for symbol in ["QQQ", "SPY"]:
    ...
    params = {"ibs_threshold": 0.25, "adx_threshold": 15.0}
    ...
    sweep = {}
    for ibs in [0.15, 0.25]:
        for adx in [15.0, 20.0, 25.0]:
            rr = strat.generate_returns(price_df, ibs_threshold=ibs, adx_threshold=adx)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"ibs={ibs},adx={adx}"] = float(sh) if sh is not None else 0.0
...
with open("scratch_results/validate_result_ibs_adx.json", "w") as f:
