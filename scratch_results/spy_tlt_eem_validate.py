...
spec = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-20_spy_tlt_eem_momentum_rotation.py"
)
...
for symbol in ["QQQ", "SPY"]:
    ...
    sweep = {}
    for lb in [21, 63, 126]:
        rr = strat.generate_returns(price_df, lookback_days=lb)
        sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
        sweep[f"lb={lb}"] = float(sh) if sh is not None else 0.0
...
with open("scratch_results/validate_result_spy_tlt_eem_rotation.json", "w") as f:
