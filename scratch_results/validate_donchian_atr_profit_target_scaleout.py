...
spec = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-20_donchian_atr_profit_target_scaleout.py"
)
...
params = {"entry_window": 35, "target1_n": 3.0, "trail_n_mult": 3.0}
...
for asset_class, symbol, loader in [
    ...
    num_trades = int(((pos > 0) & (pos.shift(1).fillna(0) == 0)).sum())
    ...
    for ew in [35, 55, 70]:
        for tn in [2.5, 3.0]:
            rr = strat.generate_returns(price_df, entry_window=ew, trail_n_mult=tn, target1_n=3.0)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"ew={ew},trail={tn}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_donchian_atr_profit_target_scaleout.json", "w") as f:
