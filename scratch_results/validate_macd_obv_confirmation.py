...
spec = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-20_macd_obv_confirmation.py"
)
...
params = {"signal_len": 9, "max_hold_days": 40}
...
for asset_class, symbol, loader in [
    ...
    for sl in [5, 9, 14]:
        for mh in [20, 40, 60]:
            rr = strat.generate_returns(price_df, signal_len=sl, max_hold_days=mh)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"sl={sl},mh={mh}"] = float(sh) if sh is not None else 0.0
