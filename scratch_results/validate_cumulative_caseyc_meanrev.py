...
spec = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-20_cumulative_caseyc_meanrev.py"
)
...
params = {"entry_threshold": 50.0, "max_hold_days": 7}
...
for asset_class, symbol, loader in [
    ...
    sweep = {}
    for et in [20.0, 30.0, 50.0]:
        for mh in [7, 10, 15]:
            rr = strat.generate_returns(price_df, entry_threshold=et, max_hold_days=mh)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"et={et},mh={mh}"] = float(sh) if sh is not None else 0.0
