...
spec = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-20_smash_day_reversal.py"
)
...
params = {"target_r_mult": 3.0, "max_hold_days": 15}
...
for asset_class, symbol, loader in [
    ...
    for tr in [1.5, 2.0, 3.0]:
        for mh in [10, 15, 25]:
            rr = strat.generate_returns(price_df, target_r_mult=tr, max_hold_days=mh)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"tr={tr},mh={mh}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_smash_day_reversal.json", "w") as f:
