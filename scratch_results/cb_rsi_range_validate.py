...
for symbol in ["QQQ", "SPY"]:
    ...
    params = {"bull_support_level": 40, "trend_window": 200}
    ...
    for bsl in [35, 40, 45]:
        for tw in [150, 200]:
            rr = strat.generate_returns(price_df, bull_support_level=bsl, trend_window=tw)
            sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
            sweep[f"bsl={bsl},tw={tw}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_constance_brown_rsi_range_shift.json", "w") as f:
