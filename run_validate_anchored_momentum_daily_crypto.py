...
spec_path = "strategies/2026-09-15_anchored_momentum_sizing_sma_trend.py"
...
for sym, params in configs.items():
    ...
    for ep in [6, 10]:
        for sens in [0.4, 0.6, 0.8]:
            r = strat.generate_returns(price_df, ema_period=ep, sma_period=params["sma_period"], sensitivity=sens, deadband=params["deadband"],
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"ep{ep}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_anchored_momentum_daily_crypto.json", "w") as f:
