...
for sym, params in configs.items():
    ...
    for bw in [14, 20]:
        for sens in [0.4, 0.6]:
            r = strat.generate_returns(price_df, bop_smoothing_window=bw, bop_sensitivity=sens, base_exposure=params["base_exposure"], deadband=params["deadband"],
                                        leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"bw{bw}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_bop_sizing_crypto_lev.json", "w") as f:
