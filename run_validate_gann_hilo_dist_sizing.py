...
spec_path = "strategies/2026-09-14_gann_hilo_dist_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("gann_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"gann_window": 5, "sensitivity": 0.5, "leverage_cap": 1.0}),
    "SPY": ("equity", {"gann_window": 5, "sensitivity": 0.5, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"gann_window": 5, "sensitivity": 0.5, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"gann_window": 5, "sensitivity": 0.5, "leverage_cap": 0.25}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for gw in [5, 10, 13]:
        r = strat.generate_returns(price_df, gann_window=gw, sensitivity=params["sensitivity"],
                                    leverage_cap=params["leverage_cap"])
        import vectorbt as vbt
        s = r.vbt.returns(freq="1D").sharpe_ratio()
        param_grid_results[f"gw{gw}"] = float(s) if s is not None else 0.0
...
with open("validators_gann_hilo_dist_sizing.json", "w") as f:
