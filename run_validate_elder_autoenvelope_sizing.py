...
spec_path = "strategies/2026-09-16_elder_autoenvelope_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("elder_ae_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"atr_mult": 2.0, "sensitivity": 0.4, "deadband": 0.40, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"atr_mult": 2.0, "sensitivity": 0.4, "deadband": 0.40, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"atr_mult": 2.5, "sensitivity": 0.6, "deadband": 0.20, "base_exposure": 0.25, "leverage_cap": 0.3}),
    "ETH/USDT": ("crypto", {"atr_mult": 2.5, "sensitivity": 0.6, "deadband": 0.20, "base_exposure": 0.25, "leverage_cap": 0.3}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for am in [1.5, 2.0, 2.5]:
        for sens in [0.4, 0.6, 0.8]:
            r = strat.generate_returns(price_df, atr_mult=am, sensitivity=sens, deadband=params["deadband"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"atr{am}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_elder_autoenvelope_sizing.json", "w") as f:
