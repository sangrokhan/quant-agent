...
Ultimate Oscillator (UO) sizing overlay + deadband -- SPY-focused
trend-window recalibration retrofit.

Hypothesis (knowledge_base/strategies_log.jsonl id 2026-09-13-087):
Direct follow-up applying the shortened-trend-window fix (RVI-082,
CMO-083, Williams%R-084, %B-085, Aroon-086) to Ultimate Oscillator sizing +
deadband (2026-09-13-079, previously QQQ-only, SPY Sharpe near-miss at
trend_window=200). Same UO sizing+deadband logic unchanged, only the SMA
trend-confirmation window (and, following Aroon-086's lesson that
trend_window alone is not always sufficient, the uo_sensitivity/deadband
params too if needed) is swept. Own-data parameter-recalibration analysis,
no new external source this iteration.
...
def _ultimate_oscillator(
    df: pd.DataFrame, fast: int = 10, mid: int = 14, slow: int = 28
) -> pd.Series:
    close = df["close"]
    high = df["high"]
    low = df["low"]
