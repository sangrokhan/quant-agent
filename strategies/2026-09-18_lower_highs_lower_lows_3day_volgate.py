"""Strategy: Lower Highs & Lower Lows 3-Day Reversal, low-vol regime-gated rescue.

Hypothesis (see knowledge_base/strategies_log.jsonl id=2026-09-18-094):
...
0.66 and parameter sensitivity 0.532 both marginally failed, BUT the
grid's isolated low-vol tercile showed Sharpe 1.80 on QQQ). This variant
adds an explicit low-vol realized-vol regime gate (20-day realized vol <=
`vol_regime_ratio` x trailing 252-day median, identical construction to
`strategies/2026-09-03_bb_meanrev_qqq_volregime.py` and the 123-pattern
rescue 2026-09-18-091), only taking the 3-day-down-streak entry when the
market IS in that low-vol regime. Source pattern itself unchanged:
...
def generate_signals(
    ...
    ``use_vol_gate`` (default True for this rescue variant): only take
    entries when trailing realized volatility is <= ``vol_regime_ratio``
    times its trailing 1-year median (low-vol regime filter) -- rescue
    per 2026-09-18-093's note that the isolated low-vol tercile alone
    showed Sharpe 1.80 vs a full-sample near-miss.
    """
    import math
    ...
    entry_condition = (streak >= consecutive_days) & low_vol_regime.fillna(False)
