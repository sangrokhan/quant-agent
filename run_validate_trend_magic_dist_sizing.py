...
spec_path = "strategies/2026-09-16_trend_magic_dist_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("tm_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"atr_coeff": 1.0, "sensitivity": 0.4, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"atr_coeff": 1.5, "sensitivity": 0.4, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"atr_coeff": 0.5, "sensitivity": 0.8, "deadband": 0.20, "base_exposure": 0.25, "leverage_cap": 0.3}),
    "ETH/USDT": ("crypto", {"atr_coeff": 1.5, "sensitivity": 0.4, "deadband": 0.20, "base_exposure": 0.25, "leverage_cap": 0.25}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for ac in [0.5, 1.0, 1.5]:
        for sens in [0.4, 0.6, 0.8]:
            r = strat.generate_returns(price_df, atr_coeff=ac, sensitivity=sens, deadband=params["deadband"],
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"ac{ac}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_trend_magic_dist_sizing.json", "w") as f:
