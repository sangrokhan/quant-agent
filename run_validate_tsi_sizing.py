...
spec_path = "strategies/2026-09-14_tsi_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("tsi_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"tsi_sensitivity": 0.6, "deadband": 0.10}),
    "SPY": ("equity", {"tsi_sensitivity": 0.4, "deadband": 0.10}),
    "BTC/USDT": ("crypto", {"tsi_sensitivity": 0.6, "deadband": 0.10}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.4, 0.6, 0.8]:
        for db in [0.05, 0.10]:
            r = strat.generate_returns(price_df, tsi_sensitivity=sens, deadband=db)
            import vectorbt as vbt
...
with open("validators_tsi_sizing.json", "w") as f:
