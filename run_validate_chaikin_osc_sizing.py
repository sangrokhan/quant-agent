...
spec_path = "strategies/2026-09-14_chaikin_osc_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("chaikin_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"sensitivity": 0.3, "deadband": 0.30}),
    "SPY": ("equity", {"sensitivity": 0.2, "deadband": 0.30}),
    "BTC/USDT": ("crypto", {"sensitivity": 0.5, "deadband": 0.15}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.2, 0.3, 0.5]:
        for db in [0.25, 0.30, 0.35]:
            r = strat.generate_returns(price_df, sensitivity=sens, deadband=db)
            import vectorbt as vbt
...
with open("validators_chaikin_osc_sizing.json", "w") as f:
