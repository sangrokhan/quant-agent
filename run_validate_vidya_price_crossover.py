...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_vidya_price_crossover.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": dict(vidya_period=10, cmo_period=14, slope_threshold=0.01, max_hold_days=40),
}
...
for sym, best_params in CONFIGS.items():
    ...
    for vp in [10, 14, 20]:
        for cp in [9, 14]:
            params = dict(vidya_period=vp, cmo_period=cp, slope_threshold=0.01, max_hold_days=40)
    ...
    with open(f"validate_result_vidya_price_crossover_{safe}.json", "w") as f:
