...
spec_path = "strategies/2026-09-14_psy_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("psy_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"psy_period": 12, "deadband": 0.35, "leverage_cap": 1.0, "sensitivity": 0.6}),
    "SPY": ("equity", {"psy_period": 8, "deadband": 0.35, "leverage_cap": 1.0, "sensitivity": 0.6}),
    "BTC/USDT": ("crypto", {"psy_period": 20, "deadband": 0.35, "leverage_cap": 0.4, "sensitivity": 0.6}),
    "ETH/USDT": ("crypto", {"psy_period": 8, "deadband": 0.25, "leverage_cap": 0.3, "sensitivity": 0.6}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for pp in [8, 12, 20]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, psy_period=pp, deadband=db,
                                        leverage_cap=params["leverage_cap"], sensitivity=params["sensitivity"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"pp{pp}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_psy_sizing.json", "w") as f:
