...
spec_mod = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-21_in_neck_line_bearish_continuation_short.py"
)
...
PARAMS = dict(trend_lookback=10, long_body_mult=1.0, target_atr_mult=3.0)
...
for symbol, loader, ac in [("QQQ", load_equity, "equity"), ("SPY", load_equity, "equity"),
    ...
    try:
        ...
        pgrid = {}
        for tbm in [1.0, 1.2, 1.5]:
            p2 = dict(PARAMS); p2["long_body_mult"] = tbm
            r2 = strat.generate_returns(df, **p2)
            sh = r2.vbt.returns(freq="D").sharpe_ratio()
            pgrid[str(tbm)] = float(sh) if sh is not None else 0.0
...
with open("validate_result_in_neck_line_short.json", "w") as f:
