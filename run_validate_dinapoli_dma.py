...
spec = importlib.util.spec_from_file_location(
    "strat_dinapoli", "strategies/2026-09-23_dinapoli_triple_dma_trend_alignment.py"
)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(long_window=40, max_hold_days=30)

cells = json.load(open("grid_cells_dinapoli_triple_dma.json"))
...
for asset_class, symbol, loader in [
    ...
    matching = [c for c in cells if c["symbol"] == symbol and c["vol_regime_label"] == "low" and c["sharpe"] is not None]
...
with open("validators_dinapoli_triple_dma.json", "w") as f:
