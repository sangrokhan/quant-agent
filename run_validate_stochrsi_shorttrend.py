...
best_params = {"trend_window": 40, "stochrsi_sensitivity": 0.5, "deadband": 0.20}
...
for symbol in ["QQQ", "SPY"]:
    ...
    for tw in [35, 40, 45]:
        for db in [0.15, 0.20, 0.25]:
            r = strat.generate_returns(price_df, trend_window=tw, stochrsi_sensitivity=0.5, deadband=db)
            try:
                import vectorbt as vbt
                sh = r.vbt.returns(freq="D").sharpe_ratio()
            except Exception:
                sh = None
            sens_grid[f"tw{tw}_db{db}"] = sh
...
with open("validators_stochrsi_shorttrend.json", "w") as f:
