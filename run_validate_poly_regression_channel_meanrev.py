...
spec_path = "strategies/2026-09-16_poly_regression_channel_meanrev.py"
mspec = importlib.util.spec_from_file_location("polyreg_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"channel_window": 30, "band_std_mult": 1.75}),
    "SPY": ("equity", {"channel_window": 30, "band_std_mult": 1.75}),
    "BTC/USDT": ("crypto", {"channel_window": 30, "band_std_mult": 1.75}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for cw in [30, 40, 50]:
        for bs in [1.25, 1.5, 1.75]:
            r = strat.generate_returns(price_df, channel_window=cw, band_std_mult=bs)
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"cw{cw}_bs{bs}"] = float(s) if s is not None else 0.0
...
with open("validators_poly_regression_channel_meanrev.json", "w") as f:
