...
spec_path = "strategies/2026-09-14_pfe_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("pfe_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"pfe_sensitivity": 0.8, "deadband": 0.25}),
    "SPY": ("equity", {"pfe_sensitivity": 0.4, "deadband": 0.20}),
    "BTC/USDT": ("crypto", {"pfe_sensitivity": 0.4, "deadband": 0.20}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.4, 0.6, 0.8]:
        for db in [0.15, 0.20, 0.25]:
            r = strat.generate_returns(price_df, pfe_sensitivity=sens, deadband=db)
            import vectorbt as vbt
...
with open("validators_pfe_sizing.json", "w") as f:
