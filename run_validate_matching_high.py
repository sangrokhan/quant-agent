...
spec = importlib.util.spec_from_file_location("strat_mh", "strategies/2026-09-21_matching_high_short.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(trend_window=50, match_tolerance_pct=0.005, target_atr_mult=3.0)
...
cells = json.load(open("grid_cells_matching_high_short.json"))
qqq_mid = [c for c in cells if c["symbol"]=="QQQ" and c["vol_regime"]=="mid" and c["sharpe"] is not None]
param_grid_results = {str(c["params"]): c["sharpe"] for c in qqq_mid}
results["parameter_sensitivity"] = check_parameter_sensitivity(param_grid_results)

for k,(p,e) in results.items():
    print(k, p, e)

with open("validators_matching_high_qqq.json","w") as f:
