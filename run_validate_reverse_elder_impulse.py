...
spec = importlib.util.spec_from_file_location(
    "strat_rei", "strategies/2026-09-22_reverse_elder_impulse_neutral_transition.py"
)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(ema_window=13, trend_window=0)

all_cells = json.load(open("grid_cells_reverse_elder_impulse.json"))
...
for symbol in ["QQQ", "SPY"]:
    ...
    for k, (p, e) in results.items():
        ...
        out[k] = {"passed": p, **e}
    json.dump(out, open(f"validators_reverse_elder_impulse_{symbol.lower()}.json", "w"), default=str)
