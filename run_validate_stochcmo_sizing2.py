...
spec_path = "strategies/2026-09-16_stochcmo_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("stochcmo_strat", spec_path)
...
configs = {
    "QQQ": {"loader": load_equity, "params": {"trend_window": 40, "cmo_period": 14, "stoch_period": 14,
             "base_exposure": 0.4, "sensitivity": 0.4, "leverage_cap": 1.0, "deadband": 0.45}},
    "SPY": {"loader": load_equity, "params": {"trend_window": 40, "cmo_period": 14, "stoch_period": 14,
             "base_exposure": 0.4, "sensitivity": 0.3, "leverage_cap": 1.0, "deadband": 0.4}},
    "BTC/USDT": {"loader": load_crypto_daily, "params": {"trend_window": 40, "cmo_period": 14, "stoch_period": 14,
             "base_exposure": 0.4, "sensitivity": 0.4, "leverage_cap": 1.0, "deadband": 0.20}},
    "ETH/USDT": {"loader": load_crypto_daily, "params": {"trend_window": 40, "cmo_period": 14, "stoch_period": 14,
             "base_exposure": 0.24, "sensitivity": 0.24, "leverage_cap": 0.6, "deadband": 0.20}},
}
...
for sym, cfg in configs.items():
    ...
    for sens_mult in [0.85, 1.0, 1.15]:
        for db_mult in [0.85, 1.0, 1.15]:
            p2 = dict(params)
            p2["sensitivity"] = round(base_sens * sens_mult, 4)
            p2["deadband"] = round(base_db * db_mult, 4)
...
with open("validators_stochcmo_sizing_final.json", "w") as f:
