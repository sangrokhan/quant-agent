...
spec_path = os.path.join(os.path.dirname(__file__), "strategies", "2026-09-13_gain_to_pain_ratio_sizing_sma_trend.py")
spec = importlib.util.spec_from_file_location("gpr_strat", spec_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

start, end = datetime(2019, 1, 1), datetime(2026, 9, 1)
best_params = {"gpr_reference": 0.8, "gpr_window": 40}

report = {}
for symbol, loader in [("SPY", load_equity), ("QQQ", load_equity), ("BTC/USDT", load_crypto_daily), ("ETH/USDT", load_crypto_daily)]:
    ...
    sens_grid = {}
    for gr in [0.8, 1.0, 1.3]:
        for gw in [40, 60]:
            r = strat.generate_returns(price_df, gpr_reference=gr, gpr_window=gw)
            try:
                import vectorbt as vbt
                sh = r.vbt.returns(freq="D").sharpe_ratio()
            except Exception:
                sh = None
            sens_grid[str((gr, gw))] = float(sh) if sh is not None else 0.0
...
with open("validators_gpr.json", "w") as f:
