...
spec_path = "strategies/2026-09-14_zigzag_maturity_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("zigzag_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"zigzag_deviation_pct": 5.0, "sensitivity": 0.5, "leverage_cap": 1.0}),
    "SPY": ("equity", {"zigzag_deviation_pct": 5.0, "sensitivity": 0.5, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"zigzag_deviation_pct": 5.0, "sensitivity": 0.5, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"zigzag_deviation_pct": 5.0, "sensitivity": 0.5, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for dev in [3.0, 5.0, 8.0]:
        r = strat.generate_returns(price_df, zigzag_deviation_pct=dev, sensitivity=params["sensitivity"],
                                    leverage_cap=params["leverage_cap"])
        import vectorbt as vbt
        s = r.vbt.returns(freq="1D").sharpe_ratio()
        param_grid_results[f"dev{dev}"] = float(s) if s is not None else 0.0
...
with open("validators_zigzag_maturity_sizing.json", "w") as f:
