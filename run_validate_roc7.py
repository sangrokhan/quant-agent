...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-21_roc7_decline_recovery.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

best_params = dict(roc_window=5, entry_threshold=-3.0, exit_threshold=4.0)

df = load_equity("QQQ", datetime(1998, 1, 1), datetime(2026, 9, 1), interval="1d")
...
cells = json.load(open("grid_cells_roc7_decline_recovery.json"))
pgr = {}
for c in cells:
...
with open("validate_result_roc7_decline_recovery_qqq.json", "w") as f:
