...
spec_path = "strategies/2026-09-14_coppock_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("coppock_strat", spec_path)
...
configs = {
    "BTC/USDT": {"trend_window": 40, "roc1_period": 14, "roc2_period": 11, "wma_window": 10,
                 "zscore_window": 100, "base_exposure": 0.15, "sensitivity": 0.6, "leverage_cap": 0.3, "deadband": 0.2},
    "ETH/USDT": {"trend_window": 40, "roc1_period": 14, "roc2_period": 11, "wma_window": 10,
                 "zscore_window": 100, "base_exposure": 0.15, "sensitivity": 0.6, "leverage_cap": 0.3, "deadband": 0.2},
}
...
for sym, params in configs.items():
    ...
    param_grid_results = {}
    for sens in [0.4, 0.5, 0.6]:
        for db in [0.10, 0.15, 0.20, 0.25]:
            p2 = dict(params)
            p2["sensitivity"] = sens
            p2["deadband"] = db
...
with open("validators_coppock_sizing_crypto_lev.json", "w") as f:
