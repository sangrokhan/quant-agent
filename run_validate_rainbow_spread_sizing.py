...
spec_path = "strategies/2026-09-14_rainbow_spread_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("rainbow_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"band_window": 15, "num_bands": 5, "sensitivity": 0.6, "leverage_cap": 1.0}),
    "SPY": ("equity", {"band_window": 15, "num_bands": 5, "sensitivity": 0.6, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"band_window": 15, "num_bands": 5, "sensitivity": 0.6, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"band_window": 15, "num_bands": 5, "sensitivity": 0.6, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for bw, nb in [(8, 5), (10, 5), (15, 5)]:
        r = strat.generate_returns(price_df, band_window=bw, num_bands=nb, sensitivity=params["sensitivity"],
                                    leverage_cap=params["leverage_cap"])
        import vectorbt as vbt
        s = r.vbt.returns(freq="1D").sharpe_ratio()
        param_grid_results[f"bw{bw}_nb{nb}"] = float(s) if s is not None else 0.0
...
with open("validators_rainbow_spread_sizing.json", "w") as f:
