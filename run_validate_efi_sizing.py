...
spec_path = "strategies/2026-09-14_efi_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("efi_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"efi_sensitivity": 0.4, "deadband": 0.15}),
    "SPY": ("equity", {"efi_sensitivity": 0.4, "deadband": 0.20}),
    "BTC/USDT": ("crypto", {"efi_sensitivity": 0.4, "deadband": 0.15}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.4, 0.6, 0.8]:
        for db in [0.15, 0.20, 0.25]:
            r = strat.generate_returns(price_df, efi_sensitivity=sens, deadband=db)
            import vectorbt as vbt
...
with open("validators_efi_sizing.json", "w") as f:
