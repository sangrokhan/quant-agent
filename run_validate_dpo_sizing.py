...
spec_path = "strategies/2026-09-14_dpo_sizing_sma_trend.py"
...
configs = {
    "QQQ": ("equity", {"dpo_window": 20, "sensitivity": 0.5, "leverage_cap": 1.0}),
    "SPY": ("equity", {"dpo_window": 20, "sensitivity": 0.5, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"dpo_window": 20, "sensitivity": 0.5, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"dpo_window": 20, "sensitivity": 0.5, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for dw in [15, 20, 30]:
        r = strat.generate_returns(price_df, dpo_window=dw, sensitivity=params["sensitivity"],
                                    leverage_cap=params["leverage_cap"])
        import vectorbt as vbt
        s = r.vbt.returns(freq="1D").sharpe_ratio()
        param_grid_results[f"dw{dw}"] = float(s) if s is not None else 0.0
...
with open("validators_dpo_sizing.json", "w") as f:
