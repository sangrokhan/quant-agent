...
spec_path = "strategies/2026-09-14_vhf_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("vhf_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"vhf_sensitivity": 0.4, "deadband": 0.20}),
    "SPY": ("equity", {"vhf_sensitivity": 0.4, "deadband": 0.15}),
    "BTC/USDT": ("crypto", {"vhf_sensitivity": 0.8, "deadband": 0.20}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.4, 0.6, 0.8]:
        for db in [0.10, 0.15, 0.20]:
            r = strat.generate_returns(price_df, vhf_sensitivity=sens, deadband=db)
            import vectorbt as vbt
...
with open("validators_vhf_sizing.json", "w") as f:
