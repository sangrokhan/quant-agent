...
configs = {
    "QQQ": ("equity", {"gap_threshold": 0.01, "max_gap_threshold": 0.05}),
    "SPY": ("equity", {"gap_threshold": 0.01, "max_gap_threshold": 0.05}),
    "BTC/USDT": ("crypto", {"gap_threshold": 0.01, "max_gap_threshold": 0.05}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    num_trades = int((signals != 0).sum())  # each nonzero day is a round-trip trade (enter+exit same day)
    cost_pass, cost_ev = check_transaction_cost_survival(returns, cost_bps_per_trade=10, num_trades=num_trades * 2, min_net_sharpe=0.5)
    ...
    param_grid_results = {}
    for gt in [0.005, 0.01, 0.02]:
        for mg in [0.05, 0.08]:
            r = strat.generate_returns(price_df, gap_threshold=gt, max_gap_threshold=mg)
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"gt{gt}_mg{mg}"] = float(s) if s is not None else 0.0
...
with open("validators_gap_fade_intraday.json", "w") as f:
