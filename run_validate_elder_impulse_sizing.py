...
spec_path = "strategies/2026-09-14_elder_impulse_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("elderimp_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"sensitivity": 0.5, "deadband": 0.40, "leverage_cap": 1.0}),
    "SPY": ("equity", {"sensitivity": 0.5, "deadband": 0.40, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"sensitivity": 0.5, "deadband": 0.25, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"sensitivity": 0.5, "deadband": 0.25, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for sens in [0.5, 1.0]:
        for db in [0.30, 0.40, 0.50] if asset_class == "equity" else [0.15, 0.25, 0.35]:
            r = strat.generate_returns(price_df, sensitivity=sens, deadband=db, leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
...
with open("validators_elder_impulse_sizing.json", "w") as f:
