...
spec = importlib.util.spec_from_file_location("strat_vorsi_pb", "strategies/2026-09-22_volume_rsi_oversold_pullback_trend.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(window=14, oversold_threshold=25, exit_threshold=60, trend_window=200, max_hold_days=30)

all_cells = json.load(open("grid_cells_volume_rsi_oversold_pullback.json"))
...
for symbol in ["SPY", "QQQ"]:
    ...
    for k,(p,e) in results.items():
        ...
        out[k] = {"passed": p, **e}
    json.dump(out, open(f"validators_volume_rsi_oversold_pullback_{symbol.lower()}.json","w"), default=str)
