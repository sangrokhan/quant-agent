...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_52wk_high_proximity_sizing.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": ("equity", dict(min_proximity=0.85, lookback_days=252, deadband=0.1)),
    "SPY": ("equity", dict(min_proximity=0.85, lookback_days=252, deadband=0.1)),
    "BTC/USDT": ("crypto", dict(min_proximity=0.9, lookback_days=252, deadband=0.05)),
    "ETH/USDT": ("crypto", dict(min_proximity=0.9, lookback_days=126, deadband=0.05)),
}
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    pgr = {}
    for mp in [0.80, 0.85, 0.90]:
        for ld in [126, 252]:
            params = dict(min_proximity=mp, lookback_days=ld, deadband=best_params["deadband"])
    ...
    with open(f"validate_result_52wk_high_proximity_sizing_{safe}.json", "w") as f:
