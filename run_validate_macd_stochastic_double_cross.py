...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_macd_stochastic_double_cross.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": ("equity", dict(sync_window=2, max_hold_days=20)),
    "SPY": ("equity", dict(sync_window=2, max_hold_days=20)),
    "BTC/USDT": ("crypto", dict(sync_window=1, max_hold_days=20)),
    "ETH/USDT": ("crypto", dict(sync_window=1, max_hold_days=20)),
}
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    for sw in [1, 2, 3]:
        for mh in [20, 40]:
            params = dict(sync_window=sw, max_hold_days=mh)
    ...
    with open(f"validate_result_macd_stochastic_double_cross_{safe}.json", "w") as f:
