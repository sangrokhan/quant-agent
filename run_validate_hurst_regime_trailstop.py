...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_hurst_regime_sma_crossover_trailstop.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": dict(hurst_window=60, hurst_threshold=0.65, trail_percent=0.05),
    "SPY": dict(hurst_window=100, hurst_threshold=0.55, trail_percent=0.05),
}

cells = json.load(open("grid_cells_hurst_regime_trailstop.json"))

all_out = {}
for sym, best_params in CONFIGS.items():
    df = load_equity(sym, datetime(2019, 1, 1), datetime(2026, 9, 1), interval="1d")
    ...
    all_out[sym] = out
    with open(f"validate_result_hurst_regime_trailstop_{sym.lower()}.json", "w") as f:
