...
strat = load_module(os.path.join(ROOT, "strategies", "2026-09-12_tqqq_adaptive_weekly_momentum_exit.py"), "weekly_strat")
...
best_params = dict(stop_pct=0.02, initial_target_pct=0.05)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for sp in [0.01, 0.015, 0.02]:
        for tp in [0.05, 0.07]:
            p2 = dict(stop_pct=sp, initial_target_pct=tp)
...
with open(os.path.join(ROOT, "validators_tqqq_weekly.json"), "w") as f:
