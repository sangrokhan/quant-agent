...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-21_lyapunov_hurst_regime_trend.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

best_params = dict(trend_window=50, lyap_threshold=0.45, hurst_threshold=0.5)

df = load_equity("QQQ", datetime(2018, 1, 1), datetime(2026, 9, 1), interval="1d")
...
try:
    ...
    # KNOWN BUG (pre-existing): vbt.utils.splitting.RangeSplitter unavailable
    # in the installed vectorbt version -- manual date-slice fallback.
    n_splits = 4
...
# param sensitivity from grid cells already run
cells = json.load(open("grid_cells_lyapunov_hurst_regime_trend.json"))
pgr = {}
for c in cells:
    if c["symbol"] == "QQQ" and c["sharpe"] is not None:
        key = str(c["params"])
...
with open("validate_result_lyapunov_hurst_regime_qqq.json", "w") as f:
