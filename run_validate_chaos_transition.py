...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-21_chaos_transition_corrdim.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

best_params = dict(trend_window=100, lyap_threshold=0.4, corr_dim_threshold=1.7)
SYM = "ETH/USDT"

df = load_crypto(SYM, datetime(2018, 1, 1), datetime(2026, 9, 1), interval="1d")
...
cells = json.load(open("grid_cells_chaos_transition_corrdim.json"))
pgr = {}
for c in cells:
...
with open("validate_result_chaos_transition_corrdim_eth.json", "w") as f:
