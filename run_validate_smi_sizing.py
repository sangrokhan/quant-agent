...
spec_path = "strategies/2026-09-14_smi_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("smi_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"smi_sensitivity": 0.4, "deadband": 0.10}),
    "SPY": ("equity", {"smi_sensitivity": 0.8, "deadband": 0.10}),
    "BTC/USDT": ("crypto", {"smi_sensitivity": 0.4, "deadband": 0.05}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.4, 0.6, 0.8]:
        for db in [0.05, 0.10]:
            r = strat.generate_returns(price_df, smi_sensitivity=sens, deadband=db)
            import vectorbt as vbt
...
with open("validators_smi_sizing.json", "w") as f:
