...
strat = load_module(os.path.join(ROOT, "strategies", "2026-09-12_ou_halflife_zscore_smoothed_fix.py"), "ou_smoothed_strat")
...
best_params = dict(lookback=40, entry_z=1.5, exit_z=0.0, min_halflife=5.0, max_halflife=30.0, max_hold_days=20, smooth_window=3)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for ez in [1.0, 1.5, 2.0]:
        for sw in [2, 3, 5]:
            p2 = dict(lookback=40, entry_z=ez, exit_z=0.0, min_halflife=5.0, max_halflife=30.0, max_hold_days=20, smooth_window=sw)
...
with open(os.path.join(ROOT, "validators_ou_halflife_smoothed.json"), "w") as f:
