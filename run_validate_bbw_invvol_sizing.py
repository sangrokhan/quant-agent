...
spec_path = "strategies/2026-09-14_bbw_invvol_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("bbw_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"bb_window": 15, "deadband": 0.15, "leverage_cap": 1.0}),
    "SPY": ("equity", {"bb_window": 20, "deadband": 0.25, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"bb_window": 20, "deadband": 0.15, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"bb_window": 25, "deadband": 0.25, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for bw in [15, 20, 25]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, bb_window=bw, deadband=db,
                                        leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"bw{bw}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_bbw_invvol_sizing.json", "w") as f:
