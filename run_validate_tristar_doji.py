...
spec_mod = importlib.util.spec_from_file_location(
    "strat_mod", "strategies/2026-09-21_tristar_doji_bullish_reversal.py"
)
...
PARAMS = dict(trend_lookback=10, doji_body_pct=0.15, reward_ratio=1.5)
results = {}
for symbol, loader, ac in [("QQQ", load_equity, "equity"), ("SPY", load_equity, "equity"),
    ...
    try:
        ...
        for db in [0.10, 0.15, 0.20]:
            p2 = dict(PARAMS); p2["doji_body_pct"] = db
            r2 = strat.generate_returns(df, **p2)
            sh = r2.vbt.returns(freq="D").sharpe_ratio()
            pgrid[str(db)] = float(sh) if sh is not None else 0.0
...
with open("validate_result_tristar_doji.json", "w") as f:
