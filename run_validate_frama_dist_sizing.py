...
spec_path = "strategies/2026-09-14_frama_dist_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("frama_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"frama_window": 10, "sensitivity": 0.7, "leverage_cap": 1.0, "deadband": 0.4}),
    "SPY": ("equity", {"frama_window": 10, "sensitivity": 0.7, "leverage_cap": 1.0, "deadband": 0.4}),
    "BTC/USDT": ("crypto", {"frama_window": 10, "sensitivity": 0.7, "leverage_cap": 0.25, "deadband": 0.20}),
    "ETH/USDT": ("crypto", {"frama_window": 10, "sensitivity": 0.7, "leverage_cap": 0.3, "deadband": 0.20}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for fw in [10, 16, 24]:
        r = strat.generate_returns(price_df, frama_window=fw, sensitivity=params["sensitivity"],
                                    leverage_cap=params["leverage_cap"], deadband=params["deadband"])
        import vectorbt as vbt
        s = r.vbt.returns(freq="1D").sharpe_ratio()
        param_grid_results[f"fw{fw}"] = float(s) if s is not None else 0.0
...
with open("validators_frama_dist_sizing.json", "w") as f:
