...
spec_path = "strategies/2026-09-14_vwmacd_hist_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("vwmacd_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"fast_window": 12, "slow_window": 35, "sensitivity": 0.5, "leverage_cap": 1.0}),
    "SPY": ("equity", {"fast_window": 12, "slow_window": 35, "sensitivity": 0.5, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"fast_window": 12, "slow_window": 35, "sensitivity": 0.5, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"fast_window": 12, "slow_window": 35, "sensitivity": 0.5, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for fw, sw in [(10, 26), (12, 35), (12, 26)]:
        r = strat.generate_returns(price_df, fast_window=fw, slow_window=sw, sensitivity=params["sensitivity"],
                                    leverage_cap=params["leverage_cap"])
        import vectorbt as vbt
        s = r.vbt.returns(freq="1D").sharpe_ratio()
        param_grid_results[f"fw{fw}_sw{sw}"] = float(s) if s is not None else 0.0
...
with open("validators_vwmacd_hist_sizing.json", "w") as f:
