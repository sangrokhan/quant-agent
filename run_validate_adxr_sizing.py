...
spec_path = "strategies/2026-09-16_adxr_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("adxr_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"adx_window": 14, "sensitivity": 0.6, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"adx_window": 10, "sensitivity": 0.4, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"adx_window": 20, "sensitivity": 0.4, "deadband": 0.20, "base_exposure": 0.2, "leverage_cap": 0.3}),
    "ETH/USDT": ("crypto", {"adx_window": 10, "sensitivity": 0.6, "deadband": 0.20, "base_exposure": 0.2, "leverage_cap": 0.25}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for aw in [10, 14, 20]:
        for sens in [0.4, 0.6, 0.8]:
            r = strat.generate_returns(price_df, adx_window=aw, sensitivity=sens, deadband=params["deadband"],
...
with open("validators_adxr_sizing.json", "w") as f:
