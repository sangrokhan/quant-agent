...
best_params = dict(entry_threshold=5, exit_threshold=60, vol_z_threshold=1.0)
...
for symbol in ["QQQ", "SPY"]:
    ...
    # Manual walk-forward split (vectorbt's RangeSplitter helper is broken
    # in this installed version -- module 'vectorbt.utils' has no attribute
    # 'splitting'; same workaround used by other iterations in this repo).
    n_splits = 4
    ...
    # param sensitivity from a small local sweep
    sens = {}
    for et in [3, 5, 7]:
        for xt in [55, 60, 65]:
            r = strat.generate_returns(price_df, entry_threshold=et, exit_threshold=xt, vol_z_threshold=1.0)
            if r.abs().sum() == 0:
                continue
            sh = r.vbt.returns(freq="D").sharpe_ratio() if hasattr(r, "vbt") else None
            import vectorbt as vbt
            sh = r.vbt.returns(freq="D").sharpe_ratio()
            sens[f"et{et}_xt{xt}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_rsi2_volz_atr_stacked.json", "w") as f:
