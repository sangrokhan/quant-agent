...
spec_path = "strategies/2026-09-16_bbkc_squeeze_rank_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("bbkc_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"rank_window": 150, "sensitivity": 0.3, "deadband": 0.50, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"rank_window": 100, "sensitivity": 0.4, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"rank_window": 60, "sensitivity": 0.4, "deadband": 0.15, "base_exposure": 0.3, "leverage_cap": 0.3}),
    "ETH/USDT": ("crypto", {"rank_window": 150, "sensitivity": 0.4, "deadband": 0.20, "base_exposure": 0.2, "leverage_cap": 0.25}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for rw in [60, 100, 150]:
        for sens in [0.3, 0.4, 0.6]:
            r = strat.generate_returns(price_df, rank_window=rw, sensitivity=sens, deadband=params["deadband"],
...
with open("validators_bbkc_squeeze_rank_sizing.json", "w") as f:
