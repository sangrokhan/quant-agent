...
spec_path = os.path.join(os.path.dirname(__file__), "strategies", "2026-09-13_cvar_targeting_sma_trend.py")
spec = importlib.util.spec_from_file_location("cvar_strat", spec_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

start, end = datetime(2019, 1, 1), datetime(2026, 9, 1)
best_params = {"target_cvar": 0.02, "cvar_window": 40}

report = {}
for asset_class, symbol, loader in [("equity", "SPY", load_equity), ("equity", "QQQ", load_equity),
                                     ("crypto", "BTC/USDT", load_crypto_daily)]:
    ...
    # approximate num_trades as number of times exposure goes from 0 to >0
    ...
    sens_grid = {}
    for tc in [0.015, 0.02, 0.03]:
        for cw in [40, 60]:
            r = strat.generate_returns(price_df, target_cvar=tc, cvar_window=cw)
            try:
                import vectorbt as vbt
                sh = r.vbt.returns(freq="D").sharpe_ratio()
            except Exception:
                sh = None
            sens_grid[str((tc, cw))] = float(sh) if sh is not None else 0.0
...
with open("validators_cvar.json", "w") as f:
