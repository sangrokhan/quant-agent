...
spec_path = "strategies/2026-09-14_imi_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("imi_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"imi_sensitivity": 0.6, "deadband": 0.15}),
    "SPY": ("equity", {"imi_sensitivity": 0.6, "deadband": 0.15}),
    "BTC/USDT": ("crypto", {"imi_sensitivity": 0.6, "deadband": 0.15}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.4, 0.6, 0.8]:
        for db in [0.05, 0.10, 0.15]:
            r = strat.generate_returns(price_df, imi_sensitivity=sens, deadband=db)
            import vectorbt as vbt
...
with open("validators_imi_sizing.json", "w") as f:
