...
spec = importlib.util.spec_from_file_location("strat_tslv", "strategies/2026-09-22_time_series_low_vol_anomaly.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(vol_window=30, low_vol_pct=0.33)

all_cells = json.load(open("grid_cells_ts_low_vol_anomaly.json"))
...
for symbol in ["QQQ", "SPY"]:
    ...
    for k,(p,e) in results.items():
        ...
        out[k] = {"passed": p, **e}
    json.dump(out, open(f"validators_ts_low_vol_{symbol.lower()}.json","w"), default=str)
