...
spec = importlib.util.spec_from_file_location("strat_vcp", "strategies/2026-09-22_variance_changepoint_trend_killswitch.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(cp_threshold=2.5, cooldown_days=5)

all_cells = json.load(open("grid_cells_variance_changepoint_killswitch.json"))
...
for symbol in ["QQQ", "SPY"]:
    ...
    sym_cells = [c for c in all_cells if c["symbol"]==symbol and c["vol_regime_label"]=="mid" and c["sharpe"] is not None]
    ...
    for k,(p,e) in results.items():
        ...
        out[k] = {"passed": p, **e}
    json.dump(out, open(f"validators_variance_cp_{symbol.lower()}.json","w"), default=str)
