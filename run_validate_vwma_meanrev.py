...
spec = importlib.util.spec_from_file_location("strat_vwma", "strategies/2026-09-22_vwma_single_line_meanrev.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(vwma_window=5, max_hold_days=20)

all_cells = json.load(open("grid_cells_vwma_single_line_meanrev.json"))
...
for symbol in ["QQQ", "SPY"]:
    ...
    for k,(p,e) in results.items():
        ...
        out[k] = {"passed": p, **e}
    json.dump(out, open(f"validators_vwma_meanrev_{symbol.lower()}.json","w"), default=str)
