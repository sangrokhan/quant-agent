...
spec_path = "strategies/2026-09-15_fcb_dist_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("fcb_strat", spec_path)
...
for sym, params in configs.items():
    ...
    param_grid_results = {}
    for zw in [50, 100, 150]:
        for sens in [0.4, 0.6, 0.8]:
            r = strat.generate_returns(price_df, zscore_window=zw, sensitivity=sens, deadband=params["deadband"],
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"zw{zw}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_fcb_sizing_daily_crypto.json", "w") as f:
