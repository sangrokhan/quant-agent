...
spec = importlib.util.spec_from_file_location("strat_rr", "strategies/2026-09-24_russell_rebalancing_seasonal.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(entry_day_of_month=23, exit_day_of_month=1)

all_results = {}
for symbol in ["IWM", "SPY"]:
    df = load_equity(symbol, datetime(2015,1,1), datetime(2026,9,1))
    ...
    print(symbol, "total return:", (1+returns).prod()-1)
    ...
    cells = json.load(open("grid_cells_russell_rebalancing.json"))
...
with open("validate_result_russell_rebalancing.json", "w") as f:
