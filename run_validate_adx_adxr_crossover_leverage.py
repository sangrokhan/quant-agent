...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_adx_adxr_crossover.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "BTC/USDT": dict(adx_period=10, max_hold_days=40, leverage_cap=0.5),
    "ETH/USDT": dict(adx_period=10, max_hold_days=20, leverage_cap=0.7),
}

all_out = {}
for sym, best_params in CONFIGS.items():
    df = load_crypto(sym, datetime(2019, 1, 1), datetime(2026, 9, 1), interval="1d")
    ...
    pgr = {}
    for lev in [0.3, 0.4, 0.5, 0.6, 0.7]:
        p = dict(best_params); p["leverage_cap"] = lev
    ...
    with open(f"validate_result_adx_adxr_crossover_leverage_{safe}.json", "w") as f:
