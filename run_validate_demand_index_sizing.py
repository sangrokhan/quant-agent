...
spec_path = "strategies/2026-09-14_demand_index_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("di_strat4", spec_path)
...
configs = {
    "QQQ": ("equity", {"sensitivity": 0.3, "deadband": 0.20}),
    "SPY": ("equity", {"sensitivity": 0.2, "deadband": 0.30}),
    "BTC/USDT": ("crypto", {"sensitivity": 0.3, "deadband": 0.15, "leverage_cap": 0.3, "base_exposure": 0.15}),
    "ETH/USDT": ("crypto", {"sensitivity": 0.7, "deadband": 0.15, "leverage_cap": 0.4, "base_exposure": 0.2}),
}
    ...
    for sens in sorted({round(sens_base - 0.2, 2), sens_base, round(sens_base + 0.2, 2)}):
        for db in [max(0.05, params["deadband"] - 0.05), params["deadband"], params["deadband"] + 0.05]:
            r = strat.generate_returns(
                price_df, sensitivity=max(0.05, sens), deadband=db,
                leverage_cap=lev, base_exposure=base_exp,
            )
            import vectorbt as vbt
...
with open("validators_demand_index_sizing.json", "w") as f:
