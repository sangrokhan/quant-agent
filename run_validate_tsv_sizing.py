...
spec_path = "strategies/2026-09-14_tsv_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("tsv_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"tsv_window": 21, "deadband": 0.25, "leverage_cap": 0.4}),
    "SPY": ("equity", {"tsv_window": 21, "deadband": 0.15, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"tsv_window": 13, "deadband": 0.15, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"tsv_window": 13, "deadband": 0.15, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for tw in [8, 13, 21]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, tsv_window=tw, deadband=db,
...
with open("validators_tsv_sizing.json", "w") as f:
