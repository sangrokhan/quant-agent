...
spec_mod_path = "strategies/2026-09-21_ascending_scallop_continuation.py"
spec = importlib.util.spec_from_file_location("strat_sc", spec_mod_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

best_params = dict(curve_window=45, min_corr=0.3, trend_window=50, vol_regime_gate=True, vol_regime_ratio=1.5)
...
for cw in [30, 45]:
    for mc in [0.2, 0.3]:
        for tw in [50, 100]:
            p = dict(curve_window=cw, min_corr=mc, trend_window=tw, vol_regime_gate=True, vol_regime_ratio=1.5)
...
with open("validate_result_scallop_volgate_qqq.json", "w") as f:
