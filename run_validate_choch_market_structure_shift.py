...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_choch_market_structure_shift.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": ("equity", dict(swing_window=5, max_hold_days=30)),
    "SPY": ("equity", dict(swing_window=3, max_hold_days=20)),
    "BTC/USDT": ("crypto", dict(swing_window=8, max_hold_days=20)),
}
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    for sw in [3, 5, 8]:
        for mh in [20, 30]:
            params = dict(swing_window=sw, max_hold_days=mh)
    ...
    with open(f"validate_result_choch_market_structure_shift_{safe}.json", "w") as f:
