...
params = {"trend_sma_window": 20, "target_height_mult": 2.0, "max_hold_days": 20}
...
with open("grid_cells_3lr.json") as f:
    cells = json.load(f)
spy_cells = [c for c in cells if c["symbol"] == "SPY" and c["sharpe"] is not None]
pg = {json.dumps(c["params"]): c["sharpe"] for c in spy_cells}
...
with open("validation_3lr_spy.json", "w") as f:
