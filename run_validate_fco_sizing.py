...
spec_path = "strategies/2026-09-16_fco_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("fco_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"fco_window": 10, "sensitivity": 0.4, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"fco_window": 20, "sensitivity": 0.4, "deadband": 0.30, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"fco_window": 20, "sensitivity": 0.6, "deadband": 0.20, "base_exposure": 0.25, "leverage_cap": 0.3}),
    "ETH/USDT": ("crypto", {"fco_window": 5, "sensitivity": 0.4, "deadband": 0.20, "base_exposure": 0.20, "leverage_cap": 0.25}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for fw in [5, 10, 20]:
        for sens in [0.4, 0.6, 0.8]:
            r = strat.generate_returns(price_df, fco_window=fw, sensitivity=sens, deadband=params["deadband"],
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"fw{fw}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_fco_sizing.json", "w") as f:
