...
spec = importlib.util.spec_from_file_location("strat_netflow", "strategies/2026-09-22_net_flow_trend_ma50.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(flow_window=20, ma_window=50, flow_entry_threshold=0.5, flow_exit_threshold=-0.5, max_hold_days=60)

all_cells = json.load(open("grid_cells_net_flow_trend_ma50.json"))

symbols = [("QQQ", "equity"), ("SPY", "equity"), ("BTC/USDT", "crypto")]
...
for symbol, asset_class in symbols:
    ...
    json.dump(out, open(f"validators_net_flow_trend_ma50_{safe_sym}.json","w"), default=str)
