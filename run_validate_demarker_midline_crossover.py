...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_demarker_midline_crossover.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": dict(dem_window=14, trend_window=100),
}

cells = json.load(open("grid_cells_demarker_midline_crossover.json"))
...
for sym, best_params in CONFIGS.items():
    ...
    all_out[sym] = out
    with open(f"validate_result_demarker_midline_crossover_{sym.lower()}.json", "w") as f:
