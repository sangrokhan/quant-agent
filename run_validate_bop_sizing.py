...
spec_path = "strategies/2026-09-14_bop_sizing_sma_trend.py"
...
configs = {
    "QQQ": ("equity", {"bop_sensitivity": 0.8, "deadband": 0.15}),
    "SPY": ("equity", {"bop_sensitivity": 0.8, "deadband": 0.25}),
    "BTC/USDT": ("crypto", {"bop_sensitivity": 0.8, "deadband": 0.05}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.4, 0.6, 0.8]:
        for db in [0.05, 0.10]:
            r = strat.generate_returns(price_df, bop_sensitivity=sens, deadband=db)
            import vectorbt as vbt
...
with open("validators_bop_sizing.json", "w") as f:
