...
best_params = {"trend_window": 32, "pb_sensitivity": 0.4, "bb_std": 2.0}
...
for symbol in ["QQQ", "SPY"]:
    ...
    sens_grid = {}
    for tw in [28, 32, 36]:
        for pbs in [0.35, 0.4, 0.45]:
            r = strat.generate_returns(price_df, trend_window=tw, pb_sensitivity=pbs, bb_std=2.0)
            try:
                import vectorbt as vbt
                sh = r.vbt.returns(freq="D").sharpe_ratio()
            except Exception:
                sh = None
            sens_grid[f"tw{tw}_pbs{pbs}"] = sh
...
with open("validators_pctb_shorttrend.json", "w") as f:
