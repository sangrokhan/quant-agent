...
spec = importlib.util.spec_from_file_location("strat_bss", "strategies/2026-09-21_bearish_stick_sandwich.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(trend_window=30, close_tolerance_pct=0.01, reward_atr_mult=3.0)
df = load_equity("QQQ", datetime(2018,1,1), datetime(2026,9,1))
...
cells = json.load(open("grid_cells_bearish_stick_sandwich.json"))
qqq_high = [c for c in cells if c["symbol"]=="QQQ" and c["vol_regime"]=="high" and c["sharpe"] is not None]
param_grid_results = {str(c["params"]): c["sharpe"] for c in qqq_high}
results["parameter_sensitivity"] = check_parameter_sensitivity(param_grid_results)

for k,(p,e) in results.items():
    print(k, p, e)

with open("validators_bearish_stick_sandwich_qqq.json","w") as f:
