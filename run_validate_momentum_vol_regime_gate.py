...
CONFIG = dict(risk_free_rate_annual=0.0, vol_threshold=0.4)
...
for symbol in ["QQQ", "SPY"]:
    ...
    grid_results = {}
    for rf in [0.0, 0.02]:
        for vt in [0.25, 0.30, 0.40]:
            r = strat.generate_returns(price_df, risk_free_rate_annual=rf, vol_threshold=vt)
            sh = r.vbt.returns(freq="D").sharpe_ratio() if len(r) else None
            if sh is not None:
                grid_results[f"rf={rf},vt={vt}"] = sh
...
with open("validate_result_momentum_vol_regime_gate.json", "w") as f:
