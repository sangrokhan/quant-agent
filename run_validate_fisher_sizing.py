...
spec_path = "strategies/2026-09-14_fisher_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("fisher_strat3", spec_path)
...
configs = {
    "QQQ": ("equity", {"sensitivity": 0.3, "deadband": 0.35, "base_exposure": 0.15}),
    "BTC/USDT": ("crypto", {"sensitivity": 0.3, "deadband": 0.15, "leverage_cap": 0.4, "base_exposure": 0.2}),
    "ETH/USDT": ("crypto", {"sensitivity": 0.2, "deadband": 0.15, "leverage_cap": 0.4, "base_exposure": 0.2}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in sorted({round(max(0.05, sens_base - 0.15), 2), sens_base, round(sens_base + 0.15, 2)}):
        for db in [max(0.05, params["deadband"] - 0.05), params["deadband"], min(0.35, params["deadband"] + 0.05)]:
            r = strat.generate_returns(price_df, sensitivity=sens, deadband=db, leverage_cap=lev, base_exposure=base_exp)
            import vectorbt as vbt
...
with open("validators_fisher_sizing.json", "w") as f:
