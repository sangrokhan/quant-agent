...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_ebsw_saturation_regime_filter.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "ETH/USDT": ("crypto", dict(saturation_bars=3, trend_window=50)),
    "QQQ": ("equity", dict(saturation_bars=3, trend_window=100)),
}

cells = json.load(open("grid_cells_ebsw_saturation_regime_filter.json"))

all_out = {}
for sym, (asset_class, best_params) in CONFIGS.items():
    loader = load_crypto if asset_class == "crypto" else load_equity
    df = loader(sym, datetime(2018, 1, 1), datetime(2026, 9, 1), interval="1d")
    ...
    all_out[sym] = out
    fname_sym = sym.replace("/", "_").lower()
    with open(f"validate_result_ebsw_saturation_regime_filter_{fname_sym}.json", "w") as f:
