...
spec_mod_path = "strategies/2026-09-23_qld_regime_rotation.py"
spec = importlib.util.spec_from_file_location("strat_qld", spec_mod_path)
...
SYMBOL = sys.argv[1] if len(sys.argv) > 1 else "QLD"
price_df = load_equity(SYMBOL, start, end)
params = dict(trend_sma=150, momentum_window=126)
...
grid_cells = json.load(open("grid_cells_qld_regime_rotation.json"))
...
with open(f"validate_result_qld_regime_{SYMBOL}.json", "w") as f:
