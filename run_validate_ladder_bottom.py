...
spec = importlib.util.spec_from_file_location("strat_lb", "strategies/2026-09-21_ladder_bottom_long.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(trend_window=50, short_body_max_pct=0.35, target_atr_mult=3.0)
...
cells = json.load(open("grid_cells_ladder_bottom_long.json"))
qqq_low = [c for c in cells if c["symbol"]=="QQQ" and c["vol_regime"]=="low" and c["sharpe"] is not None]
param_grid_results = {str(c["params"]): c["sharpe"] for c in qqq_low}
results["parameter_sensitivity"] = check_parameter_sensitivity(param_grid_results)

for k,(p,e) in results.items():
    print(k, p, e)

with open("validators_ladder_bottom_qqq.json","w") as f:
