...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_coppock_curve_zero_cross.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": dict(roc_long=294, roc_short=231, wma_period=147),
    "SPY": dict(roc_long=252, roc_short=189, wma_period=147),
}

cells = json.load(open("grid_cells_coppock_curve_zero_cross.json"))

all_out = {}
for sym, best_params in CONFIGS.items():
    df = load_equity(sym, datetime(2016, 1, 1), datetime(2026, 9, 1), interval="1d")
    ...
    all_out[sym] = out
    with open(f"validate_result_coppock_curve_zero_cross_{sym.lower()}.json", "w") as f:
