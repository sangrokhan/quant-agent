...
strat = load_module(os.path.join(ROOT, "strategies", "2026-09-12_vacc_velocity_acceleration_momentum.py"), "vacc_strat")
...
best_params = dict(lookback=14, velocity_smooth=3)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for lb in [7, 10, 14]:
        for vs in [3, 5, 8]:
            p2 = dict(lookback=lb, velocity_smooth=vs)
...
with open(os.path.join(ROOT, "validators_vacc.json"), "w") as f:
