...
spec_path = "strategies/2026-09-16_accel_bands_pos_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("accel_bands_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"ab_window": 30, "sensitivity": 0.4, "deadband": 0.40, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "SPY": ("equity", {"ab_window": 20, "sensitivity": 0.4, "deadband": 0.50, "base_exposure": 0.4, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"ab_window": 10, "sensitivity": 0.6, "deadband": 0.20, "base_exposure": 0.25, "leverage_cap": 0.3}),
    "ETH/USDT": ("crypto", {"ab_window": 10, "sensitivity": 0.6, "deadband": 0.20, "base_exposure": 0.25, "leverage_cap": 0.25}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for aw in [10, 20, 30]:
        for sens in [0.4, 0.6, 0.8]:
            r = strat.generate_returns(price_df, ab_window=aw, sensitivity=sens, deadband=params["deadband"],
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"aw{aw}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_accel_bands_pos_sizing.json", "w") as f:
