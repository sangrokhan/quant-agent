...
strat = load_module(os.path.join(ROOT, "strategies", "2026-09-12_hann_rsi_centerline_crossover.py"), "hann_strat")
...
best_params = dict(length=21, max_hold_days=45)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for lg in [10, 14, 21]:
        for mh in [20, 30, 45]:
            p2 = dict(length=lg, max_hold_days=mh)
...
with open(os.path.join(ROOT, "validators_hann_rsi.json"), "w") as f:
