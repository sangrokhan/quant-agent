...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_keltner_adx_breakout.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": ("equity", dict(kc_period=20, atr_mult=1.5, adx_threshold=20.0)),
    "SPY": ("equity", dict(kc_period=20, atr_mult=2.0, adx_threshold=20.0)),
    "BTC/USDT": ("crypto", dict(kc_period=14, atr_mult=2.5, adx_threshold=25.0)),
}

cells = json.load(open("grid_cells_keltner_adx_breakout.json"))
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    with open(f"validate_result_keltner_adx_breakout_{safe}.json", "w") as f:
