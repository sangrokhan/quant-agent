...
for sym, params in configs.items():
    ...
    for mp in [5, 10]:
        for sens in [0.4, 0.6]:
            r = strat.generate_returns(price_df, momentum_period=mp, rmi_sensitivity=sens, base_exposure=params["base_exposure"], deadband=params["deadband"],
...
with open("validators_rmi_sizing_crypto_lev.json", "w") as f:
