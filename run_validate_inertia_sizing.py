...
spec_path = "strategies/2026-09-16_inertia_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("inertia_strat", spec_path)
...
configs = {
    "QQQ": {"loader": load_equity, "params": {"trend_window": 40, "rvi_window": 10, "rvi_smooth": 14, "linreg_window": 20,
             "base_exposure": 0.4, "sensitivity": 0.6, "leverage_cap": 1.0, "deadband": 0.25}},
    "SPY": {"loader": load_equity, "params": {"trend_window": 40, "rvi_window": 10, "rvi_smooth": 14, "linreg_window": 20,
             "base_exposure": 0.4, "sensitivity": 0.6, "leverage_cap": 1.0, "deadband": 0.35}},
    "BTC/USDT": {"loader": load_crypto_daily, "params": {"trend_window": 40, "rvi_window": 10, "rvi_smooth": 14, "linreg_window": 20,
             "base_exposure": 0.2, "sensitivity": 0.3, "leverage_cap": 0.5, "deadband": 0.15}},
    "ETH/USDT": {"loader": load_crypto_daily, "params": {"trend_window": 40, "rvi_window": 10, "rvi_smooth": 14, "linreg_window": 20,
             "base_exposure": 0.2, "sensitivity": 0.15, "leverage_cap": 0.5, "deadband": 0.20}},
}
...
for sym, cfg in configs.items():
    ...
    base_sens = params["sensitivity"]
    for sens_mult in [0.7, 0.85, 1.0, 1.15, 1.3]:
        for db_mult in [0.85, 1.0, 1.15]:
            p2 = dict(params); p2["sensitivity"] = round(base_sens*sens_mult, 4); p2["deadband"] = round(base_db*db_mult, 4)
            r = strat.generate_returns(price_df, **p2)
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"sens{p2['sensitivity']}_db{p2['deadband']}"] = float(s) if s is not None else 0.0
...
with open("validators_inertia_sizing.json", "w") as f:
