...
spec_path = "strategies/2026-09-14_cg_oscillator_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("cg_strat3", spec_path)
...
configs = {
    "QQQ": ("equity", {"sensitivity": 0.5, "deadband": 0.20, "base_exposure": 0.25}),
    "SPY": ("equity", {"sensitivity": 0.3, "deadband": 0.25, "base_exposure": 0.15}),
    "BTC/USDT": ("crypto", {"sensitivity": 0.7, "deadband": 0.15, "leverage_cap": 0.4, "base_exposure": 0.2}),
    "ETH/USDT": ("crypto", {"sensitivity": 0.3, "deadband": 0.15, "leverage_cap": 0.4, "base_exposure": 0.2}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    base_exp = params.get("base_exposure", 0.5)
    ...
    for sens in sorted({round(sens_base - 0.2, 2), sens_base, round(sens_base + 0.2, 2)}):
        for db in [max(0.05, params["deadband"] - 0.05), params["deadband"], params["deadband"] + 0.05]:
            r = strat.generate_returns(price_df, sensitivity=max(0.05, sens), deadband=db, leverage_cap=lev, base_exposure=base_exp)
            import vectorbt as vbt
...
with open("validators_cg_oscillator_sizing.json", "w") as f:
