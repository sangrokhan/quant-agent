...
spec_path = "strategies/2026-09-14_cfo_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("cfo_strat", spec_path)
...
configs = {
    "BTC/USDT": ("crypto", {"cfo_sensitivity": 0.6, "deadband": 0.28, "base_exposure": 0.2, "leverage_cap": 0.35}),
    "ETH/USDT": ("crypto", {"cfo_sensitivity": 0.6, "deadband": 0.28, "base_exposure": 0.15, "leverage_cap": 0.3}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.4, 0.6, 0.8]:
        for db in [0.2, 0.24, 0.28]:
            r = strat.generate_returns(price_df, cfo_sensitivity=sens, deadband=db,
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            if s is not None and np.isfinite(s):
                param_grid_results[f"sens{sens}_db{db}"] = float(s)
...
with open("validators_cfo_sizing_crypto_retune.json", "w") as f:
