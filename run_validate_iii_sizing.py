...
spec_path = "strategies/2026-09-14_iii_bostian_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("iii_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"smooth_window": 21, "deadband": 0.15, "leverage_cap": 1.0, "sensitivity": 0.8}),
    "SPY": ("equity", {"smooth_window": 14, "deadband": 0.25, "leverage_cap": 1.0, "sensitivity": 0.6}),
    "BTC/USDT": ("crypto", {"smooth_window": 21, "deadband": 0.15, "leverage_cap": 0.4, "sensitivity": 0.6}),
    "ETH/USDT": ("crypto", {"smooth_window": 14, "deadband": 0.15, "leverage_cap": 0.3, "sensitivity": 0.8}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sw in [14, 21, 34]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, smooth_window=sw, deadband=db,
                                        leverage_cap=params["leverage_cap"], sensitivity=params["sensitivity"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"sw{sw}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_iii_sizing.json", "w") as f:
