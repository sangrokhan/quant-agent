...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-13_williams_r_sizing_shorttrend.py")
...
BEST = dict(trend_window=40, williams_window=10, wr_sensitivity=0.6)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for tw, ww, ws in product([30, 40, 50], [10, 14], [0.4, 0.6]):
        r = strat.generate_returns(df, trend_window=tw, williams_window=ww, wr_sensitivity=ws)
        sh = check_sharpe_ratio(r)[1]["value"]
        param_grid_results[f"tw={tw},ww={ww},ws={ws}"] = sh if sh is not None else 0.0
...
with open("validators_williams_shorttrend.json", "w") as f:
