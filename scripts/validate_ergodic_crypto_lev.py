...
BEST = dict(long_len=20, base_exposure=0.24, sensitivity=0.27, leverage_cap=0.3)
...
for symbol in ["BTC/USDT", "ETH/USDT"]:
    ...
    for lev in [0.25, 0.3, 0.35]:
        scale = lev
        be, se = 0.8*scale, 0.9*scale
        r = strat.generate_returns(df, long_len=20, base_exposure=be, sensitivity=se, leverage_cap=lev)
...
with open("validators_ergodic_crypto_lev.json", "w") as f:
