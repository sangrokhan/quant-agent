...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_atr_channel_breakout.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

start, end = datetime(2018, 1, 1), datetime(2026, 9, 1)
best_params = dict(baseline_window=350, atr_mult=7.0, max_hold_days=60)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for bw in [100, 200, 350]:
        for am in [3.0, 5.0, 7.0]:
            p2 = dict(best_params); p2["baseline_window"] = bw; p2["atr_mult"] = am
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_atr_channel.json"), "w") as f:
