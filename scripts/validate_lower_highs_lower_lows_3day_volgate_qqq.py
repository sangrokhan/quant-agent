...
spec_mod = importlib.util.spec_from_file_location("strat", "strategies/2026-09-18_lower_highs_lower_lows_3day_volgate.py")
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

price = load_equity("QQQ", datetime(2015,1,1), datetime(2026,9,1))

params = dict(max_hold_days=10, vol_regime_ratio=1.0)
...
cells = json.load(open("grid_cells_lower_highs_lower_lows_3day_volgate.json"))
qqq_cells = [c for c in cells if c["symbol"]=="QQQ" and c["asset_class"]=="equity"]
...
with open("validate_result_lower_highs_lower_lows_3day_volgate_qqq.json","w") as f:
