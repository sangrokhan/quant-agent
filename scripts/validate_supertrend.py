...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_supertrend_atr_trend_follow.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"atr_window": 14, "mult": 4.0}
...
for sym in ["QQQ", "SPY"]:
    ...
    for aw in [7, 10, 14]:
        for m in [2.0, 3.0, 4.0]:
            r = strat.generate_returns(df, atr_window=aw, mult=m)
            try:
                s = check_sharpe_ratio(r)[1]["value"]
            except Exception:
                s = 0.0
            param_grid_results[f"aw{aw}_m{m}"] = s if s is not None else 0.0
...
with open("/tmp/st_validation.json", "w") as f:
