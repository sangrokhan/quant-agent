...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-08_kalman_trend_crossover.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"kalman_q": 0.01, "max_hold_days": 15}
...
for sym in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for kq in [0.005, 0.01, 0.02]:
        for mh in [15, 20, 25]:
            p = dict(kalman_q=kq, max_hold_days=mh)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_kalman.json"), "w") as f:
