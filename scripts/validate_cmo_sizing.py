...
BEST = dict(cmo_window=20, base_exposure=1.0, cmo_sensitivity=0.4)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for cw, be, cs in product([10, 14, 20], [0.6, 0.8, 1.0], [0.4, 0.6, 0.8]):
        r = strat.generate_returns(df, cmo_window=cw, base_exposure=be, cmo_sensitivity=cs)
        sh = check_sharpe_ratio(r)[1]["value"]
        param_grid_results[f"cw={cw},be={be},cs={cs}"] = sh if sh is not None else 0.0
...
with open("validators_cmo_sizing.json", "w") as f:
