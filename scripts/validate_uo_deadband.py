...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-13_uo_sizing_deadband_sma_trend.py")
...
BEST = dict(deadband=0.20, uo_sensitivity=0.6, base_exposure=0.8)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for db, us, be in product([0.10, 0.15, 0.20], [0.4, 0.6], [0.8, 1.0]):
        r = strat.generate_returns(df, deadband=db, uo_sensitivity=us, base_exposure=be)
        sh = check_sharpe_ratio(r)[1]["value"]
        param_grid_results[f"db={db},us={us},be={be}"] = sh if sh is not None else 0.0
...
with open("validators_uo_deadband.json", "w") as f:
