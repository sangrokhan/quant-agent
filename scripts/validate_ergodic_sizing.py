...
BEST = dict(long_len=20, base_exposure=0.8, sensitivity=0.9)
...
for asset_class, symbols, loader in [("equity", ["QQQ","SPY"], load_equity), ("crypto", ["BTC/USDT","ETH/USDT"], load_crypto)]:
    for symbol in symbols:
        ...
        for ll, be, se in product([20,32],[0.4,0.6,0.8],[0.5,0.7,0.9]):
            r = strat.generate_returns(df, long_len=ll, base_exposure=be, sensitivity=se)
            sh = check_sharpe_ratio(r)[1]["value"]
            param_grid_results[f"ll={ll},be={be},se={se}"] = sh if sh is not None else 0.0
...
with open("validators_ergodic_sizing.json", "w") as f:
