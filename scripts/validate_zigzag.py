...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-08_zigzag_breakout_adx_confirm.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"zigzag_pct": 0.06, "adx_threshold": 20, "max_hold_days": 15}
...
for sym in ["QQQ", "SPY"]:
    ...
    for zp in [0.04, 0.06]:
        for at in [20, 25]:
            for mh in [15, 20]:
                p = dict(zigzag_pct=zp, adx_threshold=at, max_hold_days=mh)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_zigzag.json"), "w") as f:
