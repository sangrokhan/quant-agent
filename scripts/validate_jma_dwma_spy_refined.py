...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_jma_dwma_crossover.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

start, end = datetime(2018, 1, 1), datetime(2026, 9, 1)
best_params = dict(fast_span=4, slow_window=15, max_hold_days=25, atr_mult=1.0)
...
for fs in [3, 4, 5]:
    for sw in [15, 18, 20]:
        p2 = dict(fast_span=fs, slow_window=sw, max_hold_days=25, atr_mult=1.0)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_jma_dwma_spy_refined.json"), "w") as f:
