...
BEST = {"dpo_period": 20, "dpo_low_quantile": 0.1, "trend_ema_window": 50}
...
for sym in ["QQQ", "SPY"]:
    ...
    for dp in [14, 20]:
        for lq in [0.1, 0.2]:
            for tw in [50, 100]:
                p = dict(dpo_period=dp, dpo_low_quantile=lq, trend_ema_window=tw)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_dpo.json"), "w") as f:
