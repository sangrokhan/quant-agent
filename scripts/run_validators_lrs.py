...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-10_lrs_pullback_trend_filter.py")
...
best_params = dict(slope_window=10, hold_days=9)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for sw in [5, 10, 20]:
        for hd in [5, 9, 15]:
            p = dict(slope_window=sw, hold_days=hd)
            r = strat.generate_returns(df, **p)
            sh = r.vbt.returns(freq="D").sharpe_ratio()
            param_grid_results[f"sw{sw}_hd{hd}"] = float(sh) if sh is not None else None
...
with open("validators_lrs.json", "w") as f:
