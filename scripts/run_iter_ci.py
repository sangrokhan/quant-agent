...
spec_mod = importlib.util.spec_from_file_location("strat", "strategies/2026-09-18_ehlers_continuation_index.py")
...
params = dict(gama=0.8, length=40, buy_level=-0.9)
results = {}
for sym, loader in [("QQQ", load_equity), ("SPY", load_equity)]:
    ...
    # manual walk-forward (vbt.utils.splitting.RangeSplitter unavailable in installed vectorbt)
    ...
    # param sensitivity: use full-sample sharpe across the length grid (gama=0.8, buy_level=-0.9)
    pg_results = {}
    for length in [30, 40, 60]:
        p = dict(gama=0.8, length=length, buy_level=-0.9)
        ...
        try:
            ...
            s = None
        if s is not None:
            pg_results[str(p)] = float(s)
    ps_pass, ps_ev = check_parameter_sensitivity(pg_results)
...
with open("validators_ci.json","w") as f:
