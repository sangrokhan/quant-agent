...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-08_apen_entropy_trend_gate.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"trend_window": 50, "entropy_window": 15, "entropy_regime_ratio": 1.0}
...
for sym in ["QQQ", "SPY"]:
    ...
    # parameter sensitivity from grid results (equity cells for this symbol from grid_result_apen.json context)
    param_grid_results = {}
    for tw in [20, 50]:
        for ew in [15, 20]:
            for ratio in [0.8, 1.0]:
                p = dict(trend_window=tw, entropy_window=ew, entropy_regime_ratio=ratio)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_apen.json"), "w") as f:
