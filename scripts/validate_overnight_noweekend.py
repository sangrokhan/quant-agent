...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-08_overnight_return_noweekend.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"trend_window": 100, "exclude_weekend_gap": True}
...
for sym in ["QQQ", "SPY"]:
    ...
    # compare with weekend gap included (baseline -053's unconditional version)
    baseline_ret = strat.generate_returns(df, trend_window=100, exclude_weekend_gap=False)
    import vectorbt as vbt
    baseline_sharpe = baseline_ret.vbt.returns(freq="D").sharpe_ratio()

    param_grid_results = {}
    for tw in [50, 100]:
        for ew in [True, False]:
            p = dict(trend_window=tw, exclude_weekend_gap=ew)
    ...
    results[sym] = {
        ...
        "baseline_sharpe_with_weekend_gap": float(baseline_sharpe) if baseline_sharpe is not None else None,
    }

print(json.dumps(results, indent=2, default=str))
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_overnight_noweekend.json"), "w") as f:
