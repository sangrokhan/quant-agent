...
spec_mod = importlib.util.spec_from_file_location(
    ...
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies",
                 "2026-09-12_ehlers_jack_jill_crossover.py"),
)
...
for sym, BEST in BEST_BY_SYM.items():
    ...
    for p0 in [10, 20, 40]:
        ...
        try:
            ...
            sh = None
        param_grid_results[f"p0{p0}"] = float(sh) if sh == sh and sh is not None else 0.0
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "jj_validation.json"), "w") as f:
