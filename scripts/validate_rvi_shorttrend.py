...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-13_rvi_sizing_shorttrend_deadband.py")
...
BEST = dict(trend_window=40, deadband=0.15, rvi_sensitivity=0.4)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for tw, db, rs in product([30, 40, 50], [0.10, 0.15], [0.4, 0.6]):
        r = strat.generate_returns(df, trend_window=tw, deadband=db, rvi_sensitivity=rs)
        sh = check_sharpe_ratio(r)[1]["value"]
        param_grid_results[f"tw={tw},db={db},rs={rs}"] = sh if sh is not None else 0.0
...
with open("validators_rvi_shorttrend.json", "w") as f:
