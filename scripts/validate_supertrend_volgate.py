...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_supertrend_volregime_gated.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"mult": 4.0, "vol_regime_ratio": 1.2}
...
for sym in ["QQQ", "SPY"]:
    ...
    for m in [3.0, 4.0, 5.0]:
        for vr in [0.8, 1.0, 1.2]:
            r = strat.generate_returns(df, mult=m, vol_regime_ratio=vr)
            try:
                s = check_sharpe_ratio(r)[1]["value"]
            except Exception:
                s = 0.0
            param_grid_results[f"m{m}_vr{vr}"] = s if s is not None else 0.0
