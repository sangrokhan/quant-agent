...
spec = importlib.util.spec_from_file_location('strat_mod','strategies/2026-09-20_nr7_breakout_trend_filter.py')
...
params = dict(trend_sma=50, max_hold_days=10)
...
sweep = {}
for ts in [40, 50, 60, 89]:
    for mh in [8, 10, 12]:
        rr = strat.generate_returns(df, trend_sma=ts, max_hold_days=mh)
        sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
        sweep[f"trend_sma={ts},max_hold_days={mh}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_nr7_breakout_qqq.json", "w") as f:
