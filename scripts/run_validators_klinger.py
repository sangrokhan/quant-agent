...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-11_klinger_volume_oscillator_crossover.py")
...
params = dict(signal_period=13, trend_window=100, max_hold_days=60)  # best grid cell config
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for sp, tw in product([9, 13, 21], [50, 100]):
        rr = strat.generate_returns(df, signal_period=sp, trend_window=tw, max_hold_days=60)
        sh = check_sharpe_ratio(rr)[1]["value"]
        param_grid_results[f"sp={sp},tw={tw}"] = sh if sh is not None else 0.0
...
with open("validators_klinger.json", "w") as f:
