...
CONFIGS = {
    "crypto:BTC/USDT": ("BTC/USDT", dict(symbol="BTC/USDT", sensitivity=0.4, deadband=0.15, leverage_cap=0.3)),
    "crypto:ETH/USDT": ("ETH/USDT", dict(symbol="ETH/USDT", sensitivity=0.4, deadband=0.15, leverage_cap=0.25)),
}
...
grid_data = json.load(open("grid_cells_funding_rate_sizing.json"))

all_results = {}
for key, (symbol, params) in CONFIGS.items():
...
with open("validate_result_funding_rate_sizing.json", "w") as f:
