...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-08_qqe_trend_gated_crossover.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"smooth_period": 5, "max_hold_days": 10}
...
for sym in ["QQQ", "SPY"]:
    ...
    for sp in [3, 5, 8]:
        for mh in [10, 15, 20]:
            p = dict(smooth_period=sp, max_hold_days=mh)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_qqe.json"), "w") as f:
