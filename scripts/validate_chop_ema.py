...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_chop_regime_ema_crossover.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"chop_trend_thresh": 35, "ema_slow": 50}
...
for sym in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for ct in [35, 38.2, 45]:
        for es in [20, 30, 50]:
            r = strat.generate_returns(df, chop_trend_thresh=ct, ema_slow=es)
            try:
                s = check_sharpe_ratio(r)[1]["value"]
            except Exception:
                s = 0.0
            param_grid_results[f"ct{ct}_es{es}"] = s if s is not None else 0.0
