...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_pvt_signal_line_crossover_trend_filtered.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

start, end = datetime(2018, 1, 1), datetime(2026, 9, 1)
best_params = dict(signal_window=30, trend_window=200, max_hold_days=30)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for sw in [14, 21, 30]:
        for tw in [50, 200]:
            p2 = dict(best_params); p2["signal_window"] = sw; p2["trend_window"] = tw
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_pvt_signal.json"), "w") as f:
