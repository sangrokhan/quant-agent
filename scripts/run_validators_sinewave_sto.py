...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-10_ehlers_sine_wave_stochastic_zerocross.py")
...
best_params = dict(sto_period=21, sto_smoothing=8)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for sp in [21, 32, 50]:
        for ss in [3, 5, 8]:
            p = dict(sto_period=sp, sto_smoothing=ss)
            r = strat.generate_returns(df, **p)
            sh = r.vbt.returns(freq="D").sharpe_ratio()
            param_grid_results[f"sp{sp}_ss{ss}"] = float(sh) if sh is not None else None
...
with open("validators_sinewave_sto.json", "w") as f:
