...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-08_skewness_regime_trend_gate.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"trend_window": 50, "skew_window": 40, "skew_threshold": 0.0}
...
for sym in ["QQQ", "SPY"]:
    ...
    for tw in [20, 50]:
        for sw in [40, 60]:
            for st in [0.0, 0.2]:
                p = dict(trend_window=tw, skew_window=sw, skew_threshold=st)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_skew.json"), "w") as f:
