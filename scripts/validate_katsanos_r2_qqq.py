...
spec_mod = importlib.util.spec_from_file_location("strat", "strategies/2026-09-17_katsanos_r2_goldilocks_trend.py")
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

price = load_equity("QQQ", datetime(2019,1,1), datetime(2026,9,1))

params = dict(r2_period=25, max_hold_days=40, slope_min=0.0)
...
cells = json.load(open("grid_cells_katsanos_r2_goldilocks.json"))
qqq_cells = [c for c in cells if c["symbol"]=="QQQ" and c["vol_regime"]=="mid"]
pg = {str(c["params"]): c["sharpe"] for c in qqq_cells if c["sharpe"] is not None}
results["param_sensitivity"] = check_parameter_sensitivity(pg)

results["num_trades"] = num_trades
print(json.dumps(results, indent=2, default=str))
with open("validate_result_katsanos_r2_qqq.json","w") as f:
