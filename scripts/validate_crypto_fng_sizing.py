...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-20_crypto_fear_greed_sizing_sma_trend.py")
...
CONFIGS = {
    "equity:QQQ": (load_equity, "QQQ", dict(sensitivity=0.4, deadband=0.15, leverage_cap=0.45)),
    "equity:SPY": (load_equity, "SPY", dict(sensitivity=0.4, deadband=0.15, leverage_cap=0.45)),
...
def manual_walk_forward(price_df, params, n_splits=4, min_pass_fraction=0.75):
    ...
    return passed, {"metric": "walk_forward_pass_fraction", "value": pass_fraction,
                     ...
                     "note": "manual contiguous 4-split (vectorbt.utils.splitting.RangeSplitter unavailable)"}
...
for key, (loader, symbol, params) in CONFIGS.items():
    ...
    cells = json.load(open("grid_cells_crypto_fng_sizing.json"))
    pg = {}
    for c in cells:
...
with open("validate_result_crypto_fng_sizing.json", "w") as f:
