...
spec_mod = importlib.util.spec_from_file_location("strat", "strategies/2026-09-18_ift_cci_trend_gate.py")
...
params = dict(cci_length=14, wma_length=9, buy_level=-0.5, sell_level=0.5, trend_window=150, max_hold_days=30)
results = {}
for sym, loader in [("QQQ", load_equity), ("SPY", load_equity)]:
    ...
    cells = json.load(open("grid_cells_iftcci.json"))
    pg_results = {}
    for c in cells:
        if c['symbol'] == sym and c['params']['cci_length']==14:
            key = str(c['params'])
...
with open("validators_iftcci.json","w") as f:
