...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-13_cmo_sizing_deadband_sma_trend.py")
...
BEST = dict(deadband=0.10, cmo_sensitivity=0.4, base_exposure=1.0, cmo_window=20)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for db, cs, be in product([0.03, 0.05, 0.10], [0.4, 0.6], [0.8, 1.0]):
        r = strat.generate_returns(df, deadband=db, cmo_sensitivity=cs, base_exposure=be)
        sh = check_sharpe_ratio(r)[1]["value"]
        param_grid_results[f"db={db},cs={cs},be={be}"] = sh if sh is not None else 0.0
...
with open("validators_cmo_deadband.json", "w") as f:
