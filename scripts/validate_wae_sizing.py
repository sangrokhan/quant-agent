...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-14_wae_sizing_sma_trend.py")
...
BEST = dict(trend_window=40, sensitivity=0.6, deadband=0.15, zscore_window=120, leverage_cap=1.0, base_exposure=0.4)
BEST_CRYPTO = dict(trend_window=40, sensitivity=0.4, deadband=0.25, zscore_window=120, leverage_cap=0.4, base_exposure=0.2)

out_all = {}
for asset_class, symbols, loader, params in [
    ("equity", ["QQQ","SPY"], load_equity, BEST),
    ("crypto", ["BTC/USDT","ETH/USDT"], load_crypto, BEST_CRYPTO),
]:
    for symbol in symbols:
        ...
        param_grid_results = {}
        for se, db, zw in product([0.4,0.6,0.8],[0.15,0.25],[80,120]):
            r = strat.generate_returns(df, sensitivity=se, deadband=db, zscore_window=zw,
                                        leverage_cap=params["leverage_cap"], base_exposure=params["base_exposure"])
            sh = check_sharpe_ratio(r)[1]["value"]
            if sh is None or not (sh == sh) or sh in (float("inf"), float("-inf")):
                sh = 3.0
            param_grid_results[f"se={se},db={db},zw={zw}"] = sh
...
with open("validators_wae_sizing.json", "w") as f:
