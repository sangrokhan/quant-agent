...
spec = importlib.util.spec_from_file_location('strat_mod','strategies/2026-09-20_pnf_3box_double_top_breakout.py')
...
params = dict(box_size_pct=0.01, reversal_boxes=2)
...
sweep = {}
for bsp in [0.005, 0.01, 0.015, 0.02]:
    for rb in [2, 3]:
        rr = strat.generate_returns(df, box_size_pct=bsp, reversal_boxes=rb)
        sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
        sweep[f"box_size_pct={bsp},reversal_boxes={rb}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_pnf_3box_double_top_qqq.json", "w") as f:
