...
best_params = dict(fast_span=5, slow_window=20, max_hold_days=30)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for fs in [5, 7, 10]:
        for sw in [20, 30]:
            p2 = dict(fast_span=fs, slow_window=sw, max_hold_days=30)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_jma_dwma.json"), "w") as f:
