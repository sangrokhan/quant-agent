...
params = dict(trend_window=20, min_body_pct=0.3, max_hold_days=10)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for tw, mbp in product([20, 50, 100], [0.3, 0.5]):
        rr = strat.generate_returns(df, trend_window=tw, min_body_pct=mbp, max_hold_days=10)
        sh = check_sharpe_ratio(rr)[1]["value"]
        param_grid_results[f"tw={tw},mbp={mbp}"] = sh if sh is not None else 0.0
...
with open("validators_tasuki.json", "w") as f:
