...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-14_alligator_spread_sizing_sma_trend.py")
...
BEST = dict(atr_window=14, base_exposure=1.0, sensitivity=0.5, trend_window=40)
...
for asset_class, symbols, loader in [("equity", ["QQQ","SPY"], load_equity), ("crypto", ["BTC/USDT","ETH/USDT"], load_crypto)]:
    for symbol in symbols:
        ...
        param_grid_results = {}
        for aw, be, se in product([14,21],[0.4,0.6,0.8],[0.5,0.7,0.9]):
            r = strat.generate_returns(df, atr_window=aw, base_exposure=be, sensitivity=se)
            sh = check_sharpe_ratio(r)[1]["value"]
            param_grid_results[f"aw={aw},be={be},se={se}"] = sh if sh is not None else 0.0
...
with open("validators_alligator_spread_sizing.json", "w") as f:
