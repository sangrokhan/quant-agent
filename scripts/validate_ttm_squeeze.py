...
spec = importlib.util.spec_from_file_location('strat_mod','strategies/2026-09-20_ttm_squeeze_momentum_breakout.py')
...
df = load_crypto(symbol, start, end, interval="1d")
params = dict(kc_atr_mult=2.0, max_hold_days=10)
...
sweep = {}
for kc in [1.75, 2.0, 2.25]:
    for mh in [8, 10, 12]:
        rr = strat.generate_returns(df, kc_atr_mult=kc, max_hold_days=mh)
        sh = rr.vbt.returns(freq="D").sharpe_ratio() if len(rr) else None
        sweep[f"kc_atr_mult={kc},max_hold_days={mh}"] = float(sh) if sh is not None else 0.0
...
with open("validate_result_ttm_squeeze_btc.json", "w") as f:
