...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-10_williams_vixfix_percentrank_reversion.py")
...
best_params = dict(rank_lookback=20, entry_percentile=95.0)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for rl in [10, 20, 50]:
        for ep in [95.0, 98.0]:
            p = dict(rank_lookback=rl, entry_percentile=ep)
            r = strat.generate_returns(df, **p)
            sh = r.vbt.returns(freq="D").sharpe_ratio()
            param_grid_results[f"rl{rl}_ep{ep}"] = float(sh) if sh is not None else None
...
with open("validators_vixfix.json", "w") as f:
