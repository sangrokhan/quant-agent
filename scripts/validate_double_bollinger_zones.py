...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-10_double_bollinger_zones.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

start, end = datetime(2018, 1, 1), datetime(2026, 9, 1)
best_params = dict(bb_window=20, confirmation_closes=1, max_hold_days=40)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for bw in [15, 20, 25]:
        for cc in [1, 2, 3]:
            p2 = dict(bb_window=bw, confirmation_closes=cc, max_hold_days=40)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_double_bollinger_zones.json"), "w") as f:
