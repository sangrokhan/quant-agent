...
BEST = dict(fast_span=12, base_exposure=1.0, sensitivity=0.3, trend_window=40, zscore_window=200, deadband=0.25)
...
for asset_class, symbols, loader in [("equity", ["QQQ","SPY"], load_equity), ("crypto", ["BTC/USDT","ETH/USDT"], load_crypto)]:
    for symbol in symbols:
        ...
        for fs, be, se in product([12,20],[0.4,0.6,0.8],[0.5,0.7,0.9]):
            r = strat.generate_returns(df, fast_span=fs, base_exposure=be, sensitivity=se)
            sh = check_sharpe_ratio(r)[1]["value"]
            param_grid_results[f"fs={fs},be={be},se={se}"] = sh if sh is not None else 0.0
...
with open("validators_pvo_sizing.json", "w") as f:
