...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-21_piercing_pattern_midpoint_reversal.py")
...
symbol = "QQQ"
best_params = {"trend_lookback": 10, "max_hold_days": 10}
...
# Manual walk-forward: split into 4 contiguous chunks, check per-chunk Sharpe>0
n = len(df)
chunk = n // 4
wf_results = []
for i in range(4):
...
# param sensitivity across trend_lookback grid on QQQ
grid_results = {}
for tl in [5, 10, 20]:
    r = strat.generate_returns(df, trend_lookback=tl, max_hold_days=10)
    sh = r.vbt.returns(freq="D").sharpe_ratio()
    grid_results[f"trend_lookback={tl}"] = float(sh) if sh is not None else 0.0
...
with open("/tmp/piercing_pattern_validators.json", "w") as f:
