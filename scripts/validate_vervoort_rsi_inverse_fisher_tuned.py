...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-11_vervoort_rsi_inverse_fisher.py")
)
...
per_symbol_params = {
    "QQQ": {"rsi_period": 4, "trend_window": 150, "max_hold_days": 10, "long_trigger": 15, "exit_level": 40},
    "SPY": {"rsi_period": 3, "trend_window": 100, "max_hold_days": 10, "long_trigger": 10, "exit_level": 40},
}
...
for symbol, best_params in per_symbol_params.items():
    ...
    base_rp = best_params["rsi_period"]
    base_mhd = best_params["max_hold_days"]
    grid_results = {}
    for rp in sorted(set([max(2, base_rp - 1), base_rp, base_rp + 1])):
        for mhd in [max(5, base_mhd - 3), base_mhd, base_mhd + 3]:
            p = dict(best_params)
            p["rsi_period"] = rp
            p["max_hold_days"] = mhd
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_vervoort_rsi_inverse_fisher_tuned.json"), "w") as f:
