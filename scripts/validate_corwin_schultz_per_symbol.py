...
per_symbol_params = {
    "QQQ": {"stdev_mult": 0.5, "trend_window": 50, "max_hold_days": 30},
    "SPY": {"stdev_mult": 1.0, "trend_window": 100, "max_hold_days": 30},
}
...
for symbol, best_params in per_symbol_params.items():
    ...
    # param sensitivity: sweep near the tuned config
    base_sm = best_params["stdev_mult"]
    base_mhd = best_params["max_hold_days"]
    grid_results = {}
    for sm in sorted(set([max(0.1, base_sm - 0.25), base_sm, base_sm + 0.25])):
        for mhd in [base_mhd - 5, base_mhd, base_mhd + 5]:
            p = dict(best_params)
            p["stdev_mult"] = sm
            p["max_hold_days"] = mhd
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_corwin_schultz_per_symbol.json"), "w") as f:
