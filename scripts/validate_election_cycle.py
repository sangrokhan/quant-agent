...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-08_presidential_election_cycle_seasonal.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"long_cycle_years": (3, 4), "trend_window": 0}
...
for sym in ["QQQ", "SPY"]:
    ...
    wf_ev = {
        ...
        "note": "calendar-based strategy over an 8.5yr sample only spans ~2 full election cycles; walk-forward here is a weaker signal than for indicator-based strategies",
    }

    param_grid_results = {}
    for cy in [(3, 4), (3,), (4,)]:
        for tw in [0, 100]:
            p = dict(long_cycle_years=cy, trend_window=tw)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_election_cycle.json"), "w") as f:
