...
spec_mod = importlib.util.spec_from_file_location("strat", "strategies/2026-09-17_calhoun_50pct_pullback_swing.py")
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

price = load_equity("SPY", datetime(2019,1,1), datetime(2026,9,1))

params = dict(chan_length=15, ma_length=30, max_hold_days=15)
...
cells = json.load(open("grid_cells_calhoun_pullback.json"))
spy_cells = [c for c in cells if c["symbol"]=="SPY" and c["vol_regime"]=="mid"]
...
with open("validate_result_calhoun_pullback_spy.json","w") as f:
