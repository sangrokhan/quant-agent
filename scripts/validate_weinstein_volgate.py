...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-10_weinstein_stage2_volgate.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

start, end = datetime(2018, 1, 1), datetime(2026, 9, 1)
best_params = dict(sma_window=100, breakout_window=50, max_hold_days=60)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for sw in [100, 150]:
        for bw in [30, 50, 70]:
            p2 = dict(sma_window=sw, breakout_window=bw, max_hold_days=60)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_weinstein_volgate.json"), "w") as f:
