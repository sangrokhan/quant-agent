...
params = dict(entry_days_before_month_end=6, exit_days_into_month=2)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for ed, xd in product([2, 4, 6], [1, 2, 3]):
        rr = strat.generate_returns(df, entry_days_before_month_end=ed, exit_days_into_month=xd)
        sh = check_sharpe_ratio(rr)[1]["value"]
        param_grid_results[f"ed={ed},xd={xd}"] = sh if sh is not None else 0.0
...
with open("validators_tom.json", "w") as f:
