...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_52wk_high_momentum_qqq_refined.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"pct_from_high": 0.04, "trend_window": 200}
...
for sym in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for p in [0.03, 0.04, 0.05]:
        for t in [180, 200, 220]:
            r = strat.generate_returns(df, pct_from_high=p, trend_window=t)
            try:
                s = check_sharpe_ratio(r)[1]["value"]
            except Exception:
                s = 0.0
            param_grid_results[f"p{p}_t{t}"] = s if s is not None else 0.0
