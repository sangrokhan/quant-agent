...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-21_persistent_weakness_low_ibs.py")
...
symbol = "QQQ"
best_params = {"streak_days": 2, "ibs_threshold": 0.2, "trend_window": 150}
...
grid_results = {}
for it in [0.15, 0.2, 0.25]:
    r = strat.generate_returns(df, streak_days=2, ibs_threshold=it, trend_window=150)
    sh = r.vbt.returns(freq="D").sharpe_ratio()
    grid_results[f"ibs={it}"] = float(sh) if sh is not None else 0.0
...
with open("/tmp/pw_ibs_validators.json", "w") as f:
