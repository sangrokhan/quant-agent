...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_fibonacci_time_zone_reversal.py")
)
...
best_params = dict(swing_window=10, tolerance_days=2, max_hold_days=10)
...
for symbol in ["QQQ", "SPY"]:
    ...
    # param sensitivity from a mini local sweep
    param_grid_results = {}
    for sw in [10, 20, 30]:
        for td in [1, 2]:
            p2 = dict(best_params); p2["swing_window"] = sw; p2["tolerance_days"] = td
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_fib_time_zone.json"), "w") as f:
