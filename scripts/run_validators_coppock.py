...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-11_coppock_curve_zero_cross.py")
...
params = dict(roc1_days=231, roc2_days=336, wma_days=210)  # best grid cell config (equity, low-vol)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for r1, r2, w in product([231], [294, 336], [147, 210]):
        rr = strat.generate_returns(df, roc1_days=r1, roc2_days=r2, wma_days=w)
        sh = check_sharpe_ratio(rr)[1]["value"]
        param_grid_results[f"roc1={r1},roc2={r2},wma={w}"] = sh if sh is not None else 0.0
...
with open("validators_coppock_curve.json", "w") as f:
