...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-09_volume_rsi_50line_crossover.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

start, end = datetime(2018, 1, 1), datetime(2026, 9, 1)
best_params = dict(window=21, max_hold_days=30)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for w in [7, 14, 21]:
        for mh in [10, 20, 30]:
            p2 = dict(window=w, max_hold_days=mh)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_volume_rsi.json"), "w") as f:
