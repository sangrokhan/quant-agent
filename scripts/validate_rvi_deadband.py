...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-13_rvi_sizing_deadband_sma_trend.py")
...
BEST = dict(deadband=0.15, rvi_sensitivity=0.4, rvi_window=10)
...
for symbol in ["QQQ", "SPY"]:
    ...
    param_grid_results = {}
    for db, rs, rw in product([0.05, 0.10, 0.15], [0.4, 0.6, 0.8], [10, 20]):
        r = strat.generate_returns(df, deadband=db, rvi_sensitivity=rs, rvi_window=rw)
        sh = check_sharpe_ratio(r)[1]["value"]
        param_grid_results[f"db={db},rs={rs},rw={rw}"] = sh if sh is not None else 0.0
...
with open("validators_rvi_deadband.json", "w") as f:
