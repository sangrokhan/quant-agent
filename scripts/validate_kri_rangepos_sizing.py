...
BEST = dict(period=14, base_exposure=0.4, sensitivity=0.7, deadband=0.5)
...
for asset_class, symbols, loader in [("equity", ["QQQ","SPY"], load_equity), ("crypto", ["BTC/USDT","ETH/USDT"], load_crypto)]:
    for symbol in symbols:
        ...
        param_grid_results = {}
        for p, be, se in product([14,21],[0.4,0.6,0.8],[0.5,0.7,0.9]):
            r = strat.generate_returns(df, period=p, base_exposure=be, sensitivity=se, deadband=0.5)
            sh = check_sharpe_ratio(r)[1]["value"]
            param_grid_results[f"p={p},be={be},se={se}"] = sh if sh is not None else 0.0
...
with open("validators_kri_rangepos_sizing.json", "w") as f:
