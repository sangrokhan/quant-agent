...
BEST = dict(atr_window=14, base_exposure=0.25, sensitivity=0.12, trend_window=40, leverage_cap=0.25)
...
for symbol in ["BTC/USDT", "ETH/USDT"]:
    ...
    param_grid_results = {}
    for lev in [0.22, 0.25, 0.28]:
        be, se = 1.0*lev, 0.5*lev
        r = strat.generate_returns(df, atr_window=14, base_exposure=be, sensitivity=se, trend_window=40, leverage_cap=lev)
...
with open("validators_alligator_crypto_lev.json", "w") as f:
