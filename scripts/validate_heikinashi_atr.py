...
spec_mod = importlib.util.spec_from_file_location(
    "strat", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "strategies", "2026-09-11_heikinashi_atr_trailstop.py")
)
strat = importlib.util.module_from_spec(spec_mod)
spec_mod.loader.exec_module(strat)

BEST = {"trend_window": 50, "consecutive_count": 2, "atr_mult": 3.5}
...
for sym in ["QQQ", "SPY"]:
    ...
    for tw in [30, 50, 100]:
        for cc in [2, 3]:
            for am in [2.5, 3.5]:
                p = dict(trend_window=tw, consecutive_count=cc, atr_mult=am)
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "validators_heikinashi_atr.json"), "w") as f:
