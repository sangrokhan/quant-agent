...
spec = importlib.util.spec_from_file_location("strat_mod", "strategies/2026-09-17_vpt_sizing_sma_trend.py")
...
# Best-per-symbol configs from the grid sweep (scripts/run_grid_vpt_sizing.py)
configs = {
    "QQQ": dict(loader=load_equity, sym="QQQ", params=dict(trend_window=50, vpt_roc_window=20, sensitivity=0.5, deadband=0.15)),
    "SPY": dict(loader=load_equity, sym="SPY", params=dict(trend_window=40, vpt_roc_window=20, sensitivity=0.5, deadband=0.20)),
    "BTC": dict(loader=load_crypto, sym="BTC/USDT", params=dict(trend_window=50, vpt_roc_window=10, sensitivity=0.5, deadband=0.15, leverage_cap=0.3, base_exposure=0.15)),
    "ETH": dict(loader=load_crypto, sym="ETH/USDT", params=dict(trend_window=50, vpt_roc_window=10, sensitivity=0.5, deadband=0.15, leverage_cap=0.3, base_exposure=0.15)),
}
...
with open("/tmp/vpt_sizing_validators.json", "w") as f:
