...
spec_path = "strategies/2026-09-15_rmo_diff_sizing_sma_trend.py"
...
for sym, params in configs.items():
    ...
    for st2 in [20, 30]:
        for sens in [0.4, 0.6, 0.8]:
            r = strat.generate_returns(price_df, st2_span=st2, st3_span=params["st3_span"], sensitivity=sens, deadband=params["deadband"],
                                        base_exposure=params["base_exposure"], leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"st2{st2}_sens{sens}"] = float(s) if s is not None else 0.0
...
with open("validators_rmo_diff_daily_crypto.json", "w") as f:
