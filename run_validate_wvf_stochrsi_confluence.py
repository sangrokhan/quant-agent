...
configs = {
    "QQQ": ("equity", {"oversold_threshold": 25.0, "wvf_bb_std": 1.5, "max_hold_days": 15}),
    "SPY": ("equity", {"oversold_threshold": 25.0, "wvf_bb_std": 1.5, "max_hold_days": 15}),
    "BTC/USDT": ("crypto", {"oversold_threshold": 25.0, "wvf_bb_std": 1.5, "max_hold_days": 15}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    param_grid_results = {}
    for ot in [15.0, 20.0, 25.0]:
        for bb in [1.5, 2.0]:
            r = strat.generate_returns(price_df, oversold_threshold=ot, wvf_bb_std=bb, max_hold_days=15)
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"ot{ot}_bb{bb}"] = float(s) if s is not None else 0.0
...
with open("validators_wvf_stochrsi_confluence.json", "w") as f:
