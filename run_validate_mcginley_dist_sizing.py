...
spec_path = "strategies/2026-09-14_mcginley_dist_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("mcg_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"md_n": 20, "deadband": 0.25, "leverage_cap": 0.4}),
    "SPY": ("equity", {"md_n": 20, "deadband": 0.15, "leverage_cap": 0.4}),
    "BTC/USDT": ("crypto", {"md_n": 20, "deadband": 0.15, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"md_n": 14, "deadband": 0.15, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for n in [10, 14, 20]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, md_n=n, deadband=db,
                                        leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"n{n}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_mcginley_dist_sizing.json", "w") as f:
