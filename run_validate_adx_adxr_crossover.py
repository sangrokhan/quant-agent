...
CONFIGS = {
    "QQQ": ("equity", dict(adx_period=14, max_hold_days=40)),
    "SPY": ("equity", dict(adx_period=10, max_hold_days=20)),
    "BTC/USDT": ("crypto", dict(adx_period=10, max_hold_days=40)),
    "ETH/USDT": ("crypto", dict(adx_period=10, max_hold_days=20)),
}
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    for ap in [10, 14, 20]:
        for mh in [20, 40]:
            p = dict(adx_period=ap, max_hold_days=mh)
    ...
    with open(f"validate_result_adx_adxr_crossover_{safe}.json", "w") as f:
