...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-21_corwin_schultz_liquidity_trend.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

best_params = dict(fast_window=20, slow_window=100, spread_pctile_threshold=0.7)
SYM = "QQQ"

df = load_equity(SYM, datetime(2018, 1, 1), datetime(2026, 9, 1), interval="1d")
...
cells = json.load(open("grid_cells_corwin_schultz_liquidity_trend.json"))
pgr = {}
for c in cells:
...
with open("validate_result_corwin_schultz_liquidity_trend_qqq.json", "w") as f:
