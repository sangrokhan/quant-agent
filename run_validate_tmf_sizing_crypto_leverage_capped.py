...
spec_path = "strategies/2026-09-14_tmf_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("tmf_strat3", spec_path)
...
configs = {
    "BTC/USDT": ("crypto", {"sensitivity": 0.5, "deadband": 0.15, "leverage_cap": 0.4, "base_exposure": 0.2}),
    "ETH/USDT": ("crypto", {"sensitivity": 0.5, "deadband": 0.15, "leverage_cap": 0.4, "base_exposure": 0.2}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in [0.3, 0.5, 0.7]:
        for lev in [0.3, 0.4, 0.5]:
            r = strat.generate_returns(price_df, sensitivity=sens, deadband=0.15, leverage_cap=lev, base_exposure=lev * 0.5)
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"sens{sens}_lev{lev}"] = float(s) if s is not None else 0.0
...
with open("validators_tmf_sizing_crypto_leverage_capped.json", "w") as f:
