...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_butterworth_3pole_trigger_cross.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": dict(period=15, trigger_lag=2, max_hold_days=40),
    "SPY": dict(period=15, trigger_lag=2, max_hold_days=40),
}
...
for sym, best_params in CONFIGS.items():
    ...
    for p in [15, 20, 30]:
        for tl in [2, 3]:
            params = dict(period=p, trigger_lag=tl, max_hold_days=40)
    ...
    with open(f"validate_result_butterworth_3pole_trigger_cross_{safe}.json", "w") as f:
