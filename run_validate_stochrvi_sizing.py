...
spec_path = "strategies/2026-09-16_stochrvi_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("stochrvi_strat", spec_path)
...
configs = {
    "QQQ": {"loader": load_equity, "params": {"trend_window": 40, "rvi_period": 10, "rvi_smoothing": 14, "stoch_period": 14,
             "base_exposure": 0.4, "sensitivity": 0.3, "leverage_cap": 1.0, "deadband": 0.60}},
    "SPY": {"loader": load_equity, "params": {"trend_window": 100, "rvi_period": 10, "rvi_smoothing": 14, "stoch_period": 14,
             "base_exposure": 0.4, "sensitivity": 0.2, "leverage_cap": 1.0, "deadband": 0.60}},
    "BTC/USDT": {"loader": load_crypto_daily, "params": {"trend_window": 40, "rvi_period": 10, "rvi_smoothing": 14, "stoch_period": 14,
             "base_exposure": 0.4, "sensitivity": 0.4, "leverage_cap": 1.0, "deadband": 0.20}},
    "ETH/USDT": {"loader": load_crypto_daily, "params": {"trend_window": 40, "rvi_period": 10, "rvi_smoothing": 14, "stoch_period": 14,
             "base_exposure": 0.4, "sensitivity": 0.4, "leverage_cap": 1.0, "deadband": 0.20}},
}
...
for sym, cfg in configs.items():
    ...
    for sens in [0.3, 0.4, 0.5, 0.6]:
        for db in [0.15, 0.20, 0.25]:
            p2 = dict(params); p2["sensitivity"]=sens; p2["deadband"]=db
            r = strat.generate_returns(price_df, **p2)
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"sens{sens}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_stochrvi_sizing.json", "w") as f:
