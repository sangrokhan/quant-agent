...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_vortex_crossover_tpsl.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": dict(vi_period=14, atr_mult=1.5, tp_sl_ratio=1.5),
    "SPY": dict(vi_period=10, atr_mult=1.5, tp_sl_ratio=1.5),
}

cells = json.load(open("grid_cells_vortex_crossover_tpsl.json"))
...
for sym, best_params in CONFIGS.items():
    ...
    all_out[sym] = out
    with open(f"validate_result_vortex_crossover_tpsl_{sym.lower()}.json", "w") as f:
