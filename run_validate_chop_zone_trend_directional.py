...
spec_mod_path = "strategies/2026-09-22_chop_zone_trend_directional.py"
spec = importlib.util.spec_from_file_location("strat_chopzone", spec_mod_path)
...
price_df = load_crypto("ETH/USDT", start, end)
params = dict(chop_window=14, ema_window=34, confirm_bars=2)
...
grid_cells = json.load(open("grid_cells_chop_zone_trend_directional.json"))
eth_cells = [c for c in grid_cells if c["symbol"] == "ETH/USDT"]
param_grid_results = {}
for c in eth_cells:
...
with open("validate_result_chop_zone_trend_directional_eth.json", "w") as f:
