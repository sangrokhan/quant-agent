...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_ppo_trix_sync_confirmation.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "SPY": ("equity", dict(sync_window=2, max_hold_days=20)),
    "QQQ": ("equity", dict(sync_window=1, max_hold_days=20)),
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    with open(f"validate_result_ppo_trix_sync_confirmation_{safe}.json", "w") as f:
