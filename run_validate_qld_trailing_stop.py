...
SYMBOL = sys.argv[1] if len(sys.argv) > 1 else "SSO"
price_df = load_equity(SYMBOL, start, end)
params = dict(stop_loss_pct=0.08, trend_sma=150)
...
grid_cells = json.load(open("grid_cells_qld_regime_trailing_stop.json"))
...
with open(f"validate_result_qld_trailing_stop_{SYMBOL}.json", "w") as f:
