...
best_params = {"trend_window": 40, "cmf_window": 30, "cmf_sensitivity": 0.5, "deadband": 0.15}
...
for symbol in ["QQQ", "SPY"]:
    ...
    for tw in [35, 40, 45]:
        for sens in [0.4, 0.5, 0.6]:
            r = strat.generate_returns(price_df, trend_window=tw, cmf_window=30, cmf_sensitivity=sens, deadband=0.15)
            try:
                import vectorbt as vbt
                sh = r.vbt.returns(freq="D").sharpe_ratio()
            except Exception:
                sh = None
            sens_grid[f"tw{tw}_s{sens}"] = sh
...
with open("validators_cmf_shorttrend.json", "w") as f:
