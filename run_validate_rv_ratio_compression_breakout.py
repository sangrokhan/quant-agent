...
spec_path = "strategies/2026-09-16_rv_ratio_compression_breakout.py"
mspec = importlib.util.spec_from_file_location("rvratio_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"compression_threshold": 0.6, "breakout_window": 30}),
    "SPY": ("equity", {"compression_threshold": 0.6, "breakout_window": 30}),
    "BTC/USDT": ("crypto", {"compression_threshold": 0.6, "breakout_window": 30}),
    "ETH/USDT": ("crypto", {"compression_threshold": 0.6, "breakout_window": 30}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for ct in [0.6, 0.7, 0.8]:
        for bw in [10, 20, 30]:
            r = strat.generate_returns(price_df, compression_threshold=ct, breakout_window=bw)
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"ct{ct}_bw{bw}"] = float(s) if s is not None else 0.0
...
with open("validators_rv_ratio_compression_breakout.json", "w") as f:
