...
CFG = {"divergence_threshold": 1.25, "sma_window": 50, "zscore_window": 126, "max_hold_days": 20}
...
param_grid_results = {}
for dt in [1.0, 1.25, 1.5]:
    for sw in [40, 50, 60]:
        r = strat.generate_returns(price_df, divergence_threshold=dt, sma_window=sw, zscore_window=126, max_hold_days=20)
...
result = {
    "sharpe": sharpe_ev, "mdd": mdd_ev, "tc": tc_ev,
    "wf": {"metric": "walk_forward_pass_fraction", "value": wf_pass_frac, "threshold": 0.75, "per_split_passed": wf_results},
    "ps": ps_ev,
    "sharpe_pass": sharpe_ok, "mdd_pass": mdd_ok, "tc_pass": tc_ok, "wf_pass": wf_ok, "ps_pass": ps_ok,
    "num_trades": num_trades, "config": CFG,
}
print(json.dumps(result, indent=2, default=str))
with open("validate_result_move_vix_spy_rescue.json", "w") as f:
