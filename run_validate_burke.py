...
spec_path = os.path.join(os.path.dirname(__file__), "strategies", "2026-09-13_burke_ratio_sizing_sma_trend.py")
spec = importlib.util.spec_from_file_location("burke_strat", spec_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

start, end = datetime(2019, 1, 1), datetime(2026, 9, 1)
best_params = {"burke_reference": 0.3, "burke_window": 60}
...
for symbol, loader in [("SPY", load_equity), ("QQQ", load_equity), ("BTC/USDT", load_crypto_daily)]:
    ...
    sens_grid = {}
    for br in [0.3, 0.5, 0.8]:
        for bw in [60, 90]:
            r = strat.generate_returns(price_df, burke_reference=br, burke_window=bw)
            try:
                import vectorbt as vbt
                sh = r.vbt.returns(freq="D").sharpe_ratio()
            except Exception:
                sh = None
            sens_grid[str((br, bw))] = float(sh) if sh is not None else 0.0
...
with open("validators_burke.json", "w") as f:
