...
spec_path = "strategies/2026-09-14_starc_pctb_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("starc_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"starc_k": 1.0, "deadband": 0.25, "leverage_cap": 0.4}),
    "SPY": ("equity", {"starc_k": 1.5, "deadband": 0.25, "leverage_cap": 0.4}),
    "BTC/USDT": ("crypto", {"starc_k": 2.0, "deadband": 0.15, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"starc_k": 1.5, "deadband": 0.15, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for k in [1.0, 1.5, 2.0]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, starc_k=k, deadband=db,
                                        leverage_cap=params["leverage_cap"])
            import vectorbt as vbt
            s = r.vbt.returns(freq="1D").sharpe_ratio()
            param_grid_results[f"k{k}_db{db}"] = float(s) if s is not None else 0.0
...
with open("validators_starc_pctb_sizing.json", "w") as f:
