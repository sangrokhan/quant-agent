...
best_params = dict(atr_period=10, atr_mult=2.5, smooth_period=9, trend_window=50)
...
for ap in [10, 14]:
    for am in [2.0, 2.5]:
        for sp in [9, 14]:
            p = dict(atr_period=ap, atr_mult=am, smooth_period=sp, trend_window=50)
...
with open("validate_result_chande_kroll_stop_spy_v2.json", "w") as f:
