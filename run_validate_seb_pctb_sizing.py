...
spec_path = "strategies/2026-09-14_seb_pctb_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("seb_strat", spec_path)
...
configs = {
    "QQQ": ("equity", {"seb_k": 2.5, "deadband": 0.15, "leverage_cap": 0.4}),
    "SPY": ("equity", {"seb_k": 1.5, "deadband": 0.25, "leverage_cap": 1.0}),
    "BTC/USDT": ("crypto", {"seb_k": 1.5, "deadband": 0.15, "leverage_cap": 0.4}),
    "ETH/USDT": ("crypto", {"seb_k": 2.5, "deadband": 0.25, "leverage_cap": 0.4}),
}
...
for sym, (asset_class, params) in configs.items():
    ...
    for k in [1.5, 2.0, 2.5]:
        for db in [0.15, 0.25]:
            r = strat.generate_returns(price_df, seb_k=k, deadband=db,
...
with open("validators_seb_pctb_sizing.json", "w") as f:
