...
spec = importlib.util.spec_from_file_location("strat_zb", spec_mod_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

best_params = dict(box_window=30, z_threshold=1.5, vol_regime_gate=True, vol_regime_ratio=1.2)
...
for bw in [20, 30, 45]:
    for zt in [1.5, 1.75]:
        p = dict(box_window=bw, z_threshold=zt, vol_regime_gate=True, vol_regime_ratio=1.2)
...
with open("validate_result_zscore_range_box_volgate_qqq.json", "w") as f:
