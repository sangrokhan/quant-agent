...
spec_mod_path = "strategies/2026-09-23_kama200_contrarian_dipbuy_fixedhold.py"
spec = importlib.util.spec_from_file_location("strat_kama200", spec_mod_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(er_window=100, hold_days=150, slow_period=30)

start, end = datetime(2018, 1, 1), datetime(2026, 9, 1)
spy = load_equity("SPY", start, end)

returns = strat.generate_returns(spy, **params)
signals = strat.generate_signals(spy, **params)
...
try:
    wf_pass, wf_ev = check_walk_forward(spy, strat_fn, n_splits=4, min_pass_fraction=0.75)
except AttributeError:
    n_splits = 4
    pdf_sorted = (spy.set_index("timestamp") if "timestamp" in spy.columns else spy).sort_index()
...
with open("grid_cells_kama200_contrarian_dipbuy.json") as f:
    cells = json.load(f)
grid_results = {}
for c in cells:
    if c["symbol"] == "SPY" and c["vol_regime"] == "low":
        key = str(c["params"])
...
with open("validate_result_kama200_contrarian_dipbuy_spy.json", "w") as f:
