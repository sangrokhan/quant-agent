...
spec_path = "strategies/2026-09-14_kvo_sizing_sma_trend.py"
spec = importlib.util.spec_from_file_location("kvo_strat3", spec_path)
...
configs = {
    "QQQ": ("equity", {"sensitivity": 0.3, "deadband": 0.20, "base_exposure": 0.15}),
    "SPY": ("equity", {"sensitivity": 0.3, "deadband": 0.20, "base_exposure": 0.15}),
    "BTC/USDT": ("crypto", {"sensitivity": 0.5, "deadband": 0.15, "leverage_cap": 0.3, "base_exposure": 0.15}),
...
for sym, (asset_class, params) in configs.items():
    ...
    for sens in sorted({round(sens_base - 0.15, 2), sens_base, round(sens_base + 0.15, 2)}):
...
with open("validators_kvo_sizing.json", "w") as f:
