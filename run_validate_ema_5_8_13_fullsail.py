...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_ema_5_8_13_fullsail_pullback.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": ("equity", dict(pullback_band_pct=0.01, max_hold_days=10)),
    "SPY": ("equity", dict(pullback_band_pct=0.005, max_hold_days=10)),
    "BTC/USDT": ("crypto", dict(pullback_band_pct=0.0, max_hold_days=10)),
}

cells = json.load(open("grid_cells_ema_5_8_13_fullsail_pullback.json"))
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    with open(f"validate_result_ema_5_8_13_fullsail_{safe}.json", "w") as f:
