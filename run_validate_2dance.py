...
spec_mod_path = "strategies/2026-09-26_bulkowski_2dance_downtrend_reversal.py"
spec = importlib.util.spec_from_file_location("strat_2dance", spec_mod_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = {"shadow_body_ratio": 2.0, "target_height_mult": 3.0, "max_hold_days": 20}

price_df = load_crypto("BTC/USDT", start=datetime(2018, 1, 1), end=datetime(2026, 9, 1))
...
with open("grid_cells_2dance.json") as f:
    cells = json.load(f)
btc_cells = [c for c in cells if c["symbol"] == "BTC/USDT" and c["sharpe"] is not None]
pg = {json.dumps(c["params"]): c["sharpe"] for c in btc_cells}
...
with open("validation_2dance_btc.json", "w") as f:
