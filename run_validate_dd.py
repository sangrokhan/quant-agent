...
spec_path = os.path.join(os.path.dirname(__file__), "strategies", "2026-09-13_downside_deviation_sizing_sma_trend.py")
spec = importlib.util.spec_from_file_location("dd_strat", spec_path)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

start, end = datetime(2019, 1, 1), datetime(2026, 9, 1)
best_params = {"target_downside_dev": 0.015, "dd_window": 40}
...
for symbol, loader in [("SPY", load_equity), ("QQQ", load_equity), ("BTC/USDT", load_crypto_daily)]:
    ...
    sens_grid = {}
    for tdd in [0.005, 0.01, 0.015]:
        for dw in [40, 60]:
            r = strat.generate_returns(price_df, target_downside_dev=tdd, dd_window=dw)
            try:
                import vectorbt as vbt
                sh = r.vbt.returns(freq="D").sharpe_ratio()
            except Exception:
                sh = None
            sens_grid[str((tdd, dw))] = float(sh) if sh is not None else 0.0
...
with open("validators_dd.json", "w") as f:
