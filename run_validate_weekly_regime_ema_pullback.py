...
spec_mod_path = "strategies/2026-09-22_weekly_regime_ema_pullback_atr.py"
spec = importlib.util.spec_from_file_location("strat_wr", spec_mod_path)
...
params = dict(ema_window=10, atr_stop_mult=1.5, atr_tp_mult=3.0)
...
out["walk_forward"] = (bool(pf >= 0.75), {"metric": "walk_forward_pass_fraction", "value": pf, "threshold": 0.75, "n_splits": splits, "per_split_sharpe": results, "note": "manual 4-equal-slice fallback; vbt.utils.splitting unavailable"})

grid_cells = json.load(open("grid_cells_weekly_regime_ema_pullback_atr.json"))
...
with open("validate_result_weekly_regime_ema_pullback_qqq.json", "w") as f:
