...
strat = load_module(os.path.join(ROOT, "strategies", "2026-09-12_chande_trendscore_oversold_cross.py"), "trendscore_strat")
...
best_params = dict(lookback=20, oversold_level=-5.0, overbought_level=5.0, confirm_window=5, max_hold_days=20)
...
for symbol in ["QQQ", "SPY"]:
    ...
    for lb in [10, 20, 30]:
        for cw in [5, 10, 15]:
            p2 = dict(lookback=lb, oversold_level=-5.0, overbought_level=5.0, confirm_window=cw, max_hold_days=20)
...
with open(os.path.join(ROOT, "validators_chande_trendscore.json"), "w") as f:
