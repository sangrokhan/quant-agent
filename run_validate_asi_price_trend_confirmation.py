...
spec = importlib.util.spec_from_file_location("strat", "strategies/2026-09-24_asi_price_trend_confirmation.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

CONFIGS = {
    "QQQ": ("equity", dict(trend_window=50, asi_trend_window=20, limit_move_pct=0.02, max_hold_days=40)),
    "SPY": ("equity", dict(trend_window=50, asi_trend_window=20, limit_move_pct=0.02, max_hold_days=40)),
}
...
for sym, (asset_class, best_params) in CONFIGS.items():
    ...
    for tw in [30, 50, 100]:
        for atw in [10, 20]:
            params = dict(trend_window=tw, asi_trend_window=atw, limit_move_pct=0.02, max_hold_days=40)
    ...
    with open(f"validate_result_asi_price_trend_confirmation_{safe}.json", "w") as f:
