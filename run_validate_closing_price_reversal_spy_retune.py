...
configs = {
    "SPY": dict(trend_window=30, edge_pct=0.2, height_mult=1.5, max_hold_days=20),
}
...
for symbol, params in configs.items():
    ...
    param_sweep = {}
    for tw in [max(10, params["trend_window"]-10), params["trend_window"], params["trend_window"]+10]:
        for ep in [max(0.05, params["edge_pct"]-0.05), params["edge_pct"], params["edge_pct"]+0.05]:
...
with open("validate_result_closing_price_reversal_spy_retune.json", "w") as f:
