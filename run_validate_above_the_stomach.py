...
spec = importlib.util.spec_from_file_location("strat_ats", "strategies/2026-09-24_above_the_stomach_downtrend_retrace.py")
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(uptrend_window=100, retrace_lookback=5, max_hold_days=5, exit_sma_window=15)
...
cells = json.load(open("grid_cells_above_the_stomach.json"))
spy_low = [c for c in cells if c["symbol"]=="SPY" and c["sharpe"] is not None]
param_grid_results = {str(c["params"]): c["sharpe"] for c in spy_low}
results["parameter_sensitivity"] = check_parameter_sensitivity(param_grid_results)

for k,(p,e) in results.items():
    print(k, p, e)

with open("validate_result_above_the_stomach_spy.json", "w") as f:
