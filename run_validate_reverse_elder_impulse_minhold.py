...
spec = importlib.util.spec_from_file_location(
    "strat_rei_mh", "strategies/2026-09-22_reverse_elder_impulse_minhold_rescue.py"
)
strat = importlib.util.module_from_spec(spec)
spec.loader.exec_module(strat)

params = dict(min_hold_days=15)

all_cells = json.load(open("grid_cells_reverse_elder_impulse_minhold.json"))
...
for symbol in ["QQQ", "SPY"]:
    ...
    for k, (p, e) in results.items():
        ...
        out[k] = {"passed": p, **e}
    json.dump(out, open(f"validators_reverse_elder_impulse_minhold_{symbol.lower()}.json", "w"), default=str)
